 ...
 protected:
  ...
  void SetUpTestIR(const std::string& ir_file) {
    ...
    control_inspector_ = absl::make_unique<ControlInspector>(
        &mock_p4_info_manager_, ir_helper_->mid_end_refmap(),
        ir_helper_->mid_end_typemap(), &mock_switch_case_decoder_,
  ...
  void SetUpMockHeaderTypes() {
    ...
    real_table_mapper_.SetHeaderAttributes("hdr.ethernet", P4_HEADER_ETHERNET,
                                           0);
    EXPECT_CALL(mock_table_map_generator_, generated_map())
        .Times(AnyNumber())
        .WillRepeatedly(ReturnRef(real_table_mapper_.generated_map()));
...
TEST_F(ControlInspectorTest, TestIngressNameAndType) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(ingress_name, pre_test_transforms_);
...
TEST_F(ControlInspectorTest, TestEgressNameAndType) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(egress_name, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
...
TEST_F(ControlInspectorTest, TestUnknownType) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(deparse_name, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
...
TEST_F(ControlInspectorTest, TestInspectTwice) {
  ...
  ir_control =
      ir_helper_->TransformP4Control(kEgressName, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
...
TEST_F(ControlInspectorTest, TestSimpleIf) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kSimpleIf, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
...
TEST_F(ControlInspectorTest, TestBasicIfElse) {
  SetUpTestIR("control_if_test.ir.json");
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kBasicIfElse, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  EXPECT_CALL(mock_p4_info_manager_, FindTableByName(_))
      .WillRepeatedly(Return(default_mock_table_));  // Default table is OK.
  control_inspector_->Inspect(*ir_control);
...
TEST_F(ControlInspectorTest, TestNestedIfs) {
  ...
  SetUpMockHeaderTypes();
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kNestedIfs, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  EXPECT_CALL(mock_p4_info_manager_, FindTableByName(_))
      .WillRepeatedly(Return(default_mock_table_));  // Default table is OK.
  control_inspector_->Inspect(*ir_control);
...
TEST_F(ControlInspectorTest, TestNotOperator) {
  ...
  SetUpMockHeaderTypes();
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kUnaryNot, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  EXPECT_CALL(mock_p4_info_manager_, FindTableByName(_))
      .WillRepeatedly(Return(default_mock_table_));  // Default table is OK.
  control_inspector_->Inspect(*ir_control);
...
TEST_P(ControlInspectorTest, TestApplyHitMiss) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(tested_control, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
...
TEST_F(ControlInspectorTest, TestBuiltInMethodCall) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kBuiltInMethod, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
  ASSERT_TRUE(control.has_main());
...
TEST_F(ControlInspectorTest, TestFieldCompare) {
  SetUpTestIR("control_misc_test.ir.json");
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kFieldCompare, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
  ASSERT_TRUE(control.has_main());
...
TEST_F(ControlInspectorTest, TestAssignConstant) {
  SetUpTestIR("control_misc_test.ir.json");
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kAssignConstant, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
  ASSERT_TRUE(control.has_main());
...
TEST_F(ControlInspectorTest, TestDropNotValid) {
  ...
  SetUpMockHeaderTypes();
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kDropNotValid, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  control_inspector_->Inspect(*ir_control);
  const hal::P4Control& control = control_inspector_->control();
  ASSERT_TRUE(control.has_main());
...
TEST_F(ControlInspectorTest, TestNopIf) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kNopIf, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
...
TEST_F(ControlInspectorTest, TestOptimizedControl) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kOptimizeBlock, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
  ...
  std::vector<std::string> test_table_names = {kAclTableName, kLpmTable1Name,
                                               kLpmTable2Name, kLpmTable3Name};
  ...
  for (int t = 0; t < fixed_tables.tables_size(); ++t) {
    ...
    SCOPED_TRACE(
        absl::StrFormat("Expected table %s does not appear in expected"
...
TEST_F(ControlInspectorTest, TestSwitchStatement) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kPuntSwitch, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
...
TEST_F(ControlInspectorTest, TestTableHitMeterAction) {
  ...
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kMeterHit, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
...
TEST_F(ControlInspectorTest, TestTableMissElseMeterAction) {
  SetUpTestIR("control_hit_meter.ir.json");
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kMeterMissElse, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
...
TEST_F(ControlInspectorTest, TestHiddenTableMeterAction) {
  SetUpTestIR("control_misc_test2.ir.json");
  const IR::P4Control* ir_control =
      ir_helper_->TransformP4Control(kMeterHitHidden, pre_test_transforms_);
  ASSERT_TRUE(ir_control != nullptr);
...
INSTANTIATE_TEST_SUITE_P(HitMissFlag, ControlInspectorTest, ::testing::Bool());
