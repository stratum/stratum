 ...
 protected:
  ...
  const IR::P4Action* GetTestAction(const std::string& action_name) {
    for (auto action_map_iter : ir_helper_->program_inspector().actions()) {
      ...
      if (std::string(action->externalName()) == action_name)
        return action;
    }
    return nullptr;
  }
...
TEST_F(ActionDecoderTest, TestAssignConstant) {
  ...
  ActionDecoder test_decoder(&mock_table_mapper_,
  ...
  EXPECT_CALL(mock_table_mapper_, AssignActionSourceValueToField(
      kTestActionName, EqualsProto(expected_value_), "meta.color")).Times(1);
...
TEST_F(ActionDecoderTest, TestAssignMultiWidthConstant) {
  ...
  EXPECT_CALL(mock_table_mapper_, AssignActionSourceValueToField(
      kTestActionName, EqualsProto(expected_value_2), "meta.smaller_metadata"))
      .Times(1);
  EXPECT_CALL(mock_table_mapper_, AddTunnelAction(_, _)).Times(0);
  const IR::P4Action* action = GetTestAction(kTestActionName);
  ASSERT_NE(nullptr, action);
...
TEST_F(ActionDecoderTest, TestAssignParam) {
  ...
  EXPECT_CALL(mock_table_mapper_, AddAction(kTestActionName)).Times(1);
  EXPECT_CALL(mock_table_mapper_, AssignActionSourceValueToField(
      kTestActionName, EqualsProto(expected_value_),
      "hdr.ethernet.dstAddr")).Times(1);
  EXPECT_CALL(mock_table_mapper_, AddTunnelAction(_, _)).Times(0);
  const IR::P4Action* action = GetTestAction(kTestActionName);
  ASSERT_NE(nullptr, action);
  test_decoder.ConvertActionBody(kTestActionName, action->body->components);
...
TEST_F(ActionDecoderTest, TestAssignHeaderCopy) {
  ...
  EXPECT_CALL(mock_table_mapper_, AddAction(kTestActionName)).Times(1);
  EXPECT_CALL(mock_table_mapper_, AssignHeaderToHeader(
      kTestActionName, EqualsProto(expected_value_), "hdr.ethernet2")).Times(1);
  ...
  EXPECT_CALL(mock_table_mapper_, AddTunnelAction(
      kTestActionName, EqualsProto(expected_encap))).Times(1);
  const IR::P4Action* action = GetTestAction(kTestActionName);
  ASSERT_NE(nullptr, action);
  test_decoder.ConvertActionBody(kTestActionName, action->body->components);
}
...
TEST_F(ActionDecoderTest, TestHeaderEncapStatement) {
  ...
  EXPECT_CALL(mock_table_mapper_, AddTunnelAction(
      kTestActionName, EqualsProto(expected_tunnel_action))).Times(1);
  test_decoder.ConvertActionBody(kTestActionName, action->body->components);
}
...
TEST_F(ActionDecoderTest, TestTwoNestedBlocks) {
  ...
  EXPECT_CALL(mock_table_mapper_, AssignActionSourceValueToField(
      kTestActionName, EqualsProto(expected_value_), "meta.smaller_metadata"))
      .Times(1);
