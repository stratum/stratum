...
TEST_F(FanDatasourceTest, InitializeFANWithEmptyInfo) {
  ...
  EXPECT_CALL(*onlp_wrapper_mock_, GetFanInfo(oid_))
      .Times(2)
      .WillRepeatedly(Return(FanInfo(mock_fan_info)));

  ::util::StatusOr<std::shared_ptr<OnlpFanDataSource>> result =
      OnlpFanDataSource::Make(id_, onlp_wrapper_mock_.get(), nullptr);
  ASSERT_OK(result);
...
TEST_F(FanDatasourceTest, SetFanData) {
  ...
  EXPECT_CALL(*onlp_wrapper_mock_, SetFanPercent(oid_, 3333))
  ...
  EXPECT_CALL(*onlp_wrapper_mock_, SetFanRpm(oid_, 4444))
      .WillOnce(Return(::util::OkStatus()));
  ...
  EXPECT_CALL(*onlp_wrapper_mock_, SetFanDir(oid_, FanDir::FAN_DIR_B2F))
      .WillOnce(Return(::util::OkStatus()));
