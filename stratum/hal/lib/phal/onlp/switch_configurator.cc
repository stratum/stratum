...
::util::Status OnlpSwitchConfigurator::CreateDefaultConfig(
  ...
  ASSIGN_OR_RETURN(oids, onlp_interface_->GetOidList(ONLP_OID_TYPE_FLAG_SFP));
...
::util::Status OnlpSwitchConfigurator::ConfigurePhalDB(
  ...
  // Add Fans
  for (int fan_tray_id = 0; fan_tray_id < phal_config.fan_trays_size();
       fan_tray_id++) {
  ...
  // Add PSUs
  for (int psu_tray_id = 0; psu_tray_id < phal_config.psu_trays_size();
       psu_tray_id++) {
  ...
  // Add LEDs
  for (int group_id = 0; group_id < phal_config.led_groups_size(); group_id++) {
  ...
  // Add Thermals
  for (int group_id = 0; group_id < phal_config.thermal_groups_size();
       group_id++) {
...
::util::StatusOr<OidInfo> OnlpSwitchConfigurator::GetOidInfo(
  ...
  if (!result.ok()) {
    LOG(ERROR) << "failed to GetOidInfo for " << to_string(oid) << ": "
    ...
    std::string err_msg =
        "Failed to get oid info for oid: " + to_string(oid) +
...
::util::Status OnlpSwitchConfigurator::AddPort(
    int card_id, int port_id, MutableAttributeGroup* mutable_card,
  ...
  if (sfp_id_map_[port_id]) {
    RETURN_ERROR(ERR_INVALID_PARAM) << "duplicate sfp id: " << port_id;
  }
  ...
  switch (config.physical_port_type()) {
...

    {
      ...
      ASSIGN_OR_RETURN(auto datasource, OnlpSfpDataSource::Make(
                                            port_id, onlp_interface_, cache));

      // Create an SFP Configurator
      ASSIGN_OR_RETURN(
          auto configurator,
          OnlpSfpConfigurator::Make(port_id, datasource, sfp, onlp_interface_));
...
::util::Status OnlpSwitchConfigurator::AddFan(
  ...
  // Check to make sure we haven't already added this id
  if (fan_id_map_[id]) {
    RETURN_ERROR(ERR_INVALID_PARAM) << "duplicate fan id: " << id;
  }
  fan_id_map_[id] = true;
...
::util::Status OnlpSwitchConfigurator::AddPsu(
  ...
  // Check to make sure we haven't already added this id
  if (psu_id_map_[id]) {
    RETURN_ERROR(ERR_INVALID_PARAM) << "duplicate psu id: " << id;
  }
  psu_id_map_[id] = true;
...
::util::Status OnlpSwitchConfigurator::AddLed(
  ...
  // Check to make sure we haven't already added this id
  if (led_id_map_[id]) {
    RETURN_ERROR(ERR_INVALID_PARAM) << "duplicate led id: " << id;
  }
  led_id_map_[id] = true;
...
::util::Status OnlpSwitchConfigurator::AddThermal(
  ...
  if (thermal_id_map_[id]) {
    RETURN_ERROR(ERR_INVALID_PARAM) << "duplicate thermal id: " << id;
  }
