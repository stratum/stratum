...
class OnlpSfpConfiguratorTest : public ::testing::Test {
 public:
   ...
   ::util::Status SetupAttributeDatabase1() {
     ...
     ASSIGN_OR_RETURN(database_,
        AttributeDatabase::MakePhalDB(std::move(switch_configurator)));
   ...
   ::util::Status SetupSfpConfigurator() {
     ...
     ASSIGN_OR_RETURN(auto cache,
        CachePolicyFactory::CreateInstance(::stratum::hal::TIMED_CACHE, 2));
...
TEST_F(OnlpSfpConfiguratorTest, AddSfpTwice) {
    ...
    std::string error_message = "sfp id "+std::to_string(id_)+" already added";
