...
class TaiPhal final : public PhalBackendInterface {
 public:
  ...
  // For event callbacks?
  // ::util::Status RegisterTaiEventCallback(TaiEventCallback* callback)
 ...
 private:
  ...
  ::util::Status Initialize(TaiInterface* tai_interface)
      LOCKS_EXCLUDED(config_lock_);
  ...
  TaiInterface* tai_interface_ GUARDED_BY(config_lock_);

  //   // Owned by the class.
  //   std::unique_ptr<TaiEventHandler> tai_event_handler_
