...
struct PathEntry {
  ...
  std::string name;
  int index = 0;
  bool indexed = false;  // This is a repeated attribute group.
  bool all = false;  // Repeated group, but ignore index and fetch all indices.
  bool terminal_group = false;  // Fetch everything below this attribute group.
...
class Query {
 public:
  ...
  // messages are sent whenever the result of the query changes, with a
  // guarantee that no longer than the given polling_interval will elapse before
  // a change is noticed. If no change has occurred after the polling interval,
  // a message is not sent. If a message would be sent by the channel buffer is
...
class AttributeDatabaseInterface {
 public:
  ...
  // change.
  virtual ::util::Status Set(
      const std::vector<std::tuple<Path, Attribute>>& values) = 0;
