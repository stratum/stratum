...
::util::Status BcmSdkWrapper::InitializeUnit(int unit, bool warm_boot) {
  ...
  for (auto const& p : configured_ports) {
    ...
    auto cl1 =
        absl::MakeCleanup([entry_hdl]() { bcmlt_entry_free(entry_hdl); });
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, p.first));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(
        entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
...
::util::Status BcmSdkWrapper::CreateKnetIntf(int unit, int vlan,
  ...
  // into the ingress pipeline But that adds VLAN tags to direct packet_outs.
  // Maybe if there is a way to stip outgoing VLAN tags. netif.vlan = vlan > 0 ?
  // vlan : kDefaultVlan; // TODO: Do we want VLAN tags on packetIO Tx packets?
