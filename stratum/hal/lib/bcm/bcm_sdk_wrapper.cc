...
// All rights reserved. The term "Broadcom" refers to Broadcom Inc. and/or its
// subsidiaries.
//
...
static absl::flat_hash_map<uint8, std::vector<int>>
    multicast_group_id_to_replicas_ = {};
...
::util::StatusOr<std::string> dump_rxpmd_header(int unit, int netif_id,
...
extern "C" int packet_receive_callback(int unit, int netif_id,
                                       bcmpkt_packet_t* packet, void* arg) {
...
std::string PrintL3EgressIntf(l3_intf_object_t l3_intf_obj,
                              int egress_intf_id) {
  uint8 mac[6];
  std::stringstream buffer;
...
// TODO(unknown): Replace InUseMap with an array or vector, but not
// std::vector<bool>!
          ...
          if ((di->device_id == dev->id.device_id) &&
            ...
            if (u != unit) {
              return MAKE_ERROR(ERR_INTERNAL)
                     ...
                     << dev->name << " found on PCI bus " << pci_bus
                     << ", PCI slot " << pci_slot
...
::util::Status BcmSdkWrapper::InitializeUnit(int unit, bool warm_boot) {
  ...
  // IFP - group
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_ING_GRP_TEMPLATEs, &table_min, &table_max));
  InUseMap ifp_groups;
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap vfp_groups;
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_VLAN_GRP_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap efp_groups;
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_EGR_GRP_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap ifp_rules;
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_ING_RULE_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap vfp_rules;
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_VLAN_RULE_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap efp_rules;
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_EGR_RULE_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_ING_POLICY_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_VLAN_POLICY_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_ERROR(
      GetTableLimits(unit, FP_EGR_POLICY_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap ifp_meters;
  RETURN_IF_ERROR(
      GetTableLimits(unit, METER_FP_ING_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  InUseMap efp_meters;
  RETURN_IF_ERROR(
      GetTableLimits(unit, METER_FP_EGR_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
  ...
  for (const auto& p : configured_ports) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_add(
        entry_hdl, STATEs, p.first, vlan_stg_str, 1));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
      entry_hdl, EGR_MEMBER_PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
      entry_hdl, ING_MEMBER_PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
      entry_hdl, UNTAGGED_MEMBER_PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
  ...
  for (const auto& p : configured_ports) {
    ...
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, VLAN_ING_TAG_ACTION_PROFILE_IDs, 1));
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, ING_OVIDs, kDefaultVlan));
    RETURN_IF_BCM_ERROR(
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
      entry_hdl, INTERVALs, FLAGS_port_counters_interval_in_usec));
...
::util::Status BcmSdkWrapper::SetPortOptions(int unit, int port,
  ...
  if (options.enabled()) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, port));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
        entry_hdl, ENABLEs, (options.enabled() == TRI_STATE_TRUE ? 1 : 0)));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(
        entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  }
  ...
  if (options.blocked()) {
    ...
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_add(
        entry_hdl, STATEs, port,
        (options.blocked() == TRI_STATE_TRUE ? &block : &forward), 1));
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(
          entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
    } else {
...
::util::Status BcmSdkWrapper::GetPortOptions(int unit, int port,
  ...
  if (entry_info.status == SHR_E_NONE) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_get(
          entry_hdl, LANE_MAPs, 0, &lanemap_array, 1, &actual_count));
      ...
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_get(
          entry_hdl, SPEED_MAXs, 0, &speed_array, 1, &actual_count));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_get(
      entry_hdl, STATEs, 0, sym_res, 140, &actual_count));
...
::util::Status BcmSdkWrapper::GetPortCounters(int unit, int port,
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, RX_FCS_ERR_PKTs, &value));
...
::util::Status BcmSdkWrapper::StartLinkscan(int unit) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_table_subscribe(unit, LM_LINK_STATEs,
                                            &sdk_linkscan_callback, NULL));
...
BcmSdkWrapper::GetPortLinkscanMode(int unit, int port) {
  ...
  BcmPortOptions::LinkscanMode linkscan_mode =
      BcmPortOptions::LINKSCAN_MODE_UNKNOWN;
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  // Check if the unit is valid
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
...
::util::StatusOr<int> BcmSdkWrapper::FindOrCreateL3RouterIntf(int unit,
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs,
                                            (vlan > 0 ? vlan : kDefaultVlan)));
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
...
::util::Status BcmSdkWrapper::ModifyL3CpuEgressIntf(int unit,
  ...
  if (it != l3_egress_intf->end()) {
    if (!it->second) {
      return MAKE_ERROR(ERR_INTERNAL)
             << "L3 Egress interface " << egress_intf_id << " is not created.";
    }
  } else {
    return MAKE_ERROR(ERR_INTERNAL)
           << "Invalid L3 Egress interface " << egress_intf_id << ".";
  }
...
::util::StatusOr<int> BcmSdkWrapper::FindOrCreateEcmpEgressIntf(
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
      entry_hdl, NHOP_IDs, 0, members_array, members_count));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  // consume slot
...
::util::Status BcmSdkWrapper::ModifyEcmpEgressIntf(
  ...
  if (it != ecmp_intfs->end()) {
    if (!it->second) {
      return MAKE_ERROR(ERR_INTERNAL) << "ECMP egress interface "
                                      << egress_intf_id << " is not created.";
    }
  } else {
    return MAKE_ERROR(ERR_INTERNAL)
           << "Invalid ECMP egress interface " << egress_intf_id << ".";
  }
...
::util::Status BcmSdkWrapper::AddL3RouteIpv4(int unit, int vrf, uint32 subnet,
  ...
  if (class_id > 0) {
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV4_UC_ROUTE_VRFs,
                                            CLASS_IDs, &min, &max));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
      entry_hdl, IPV4_MASKs, (!subnet ? 0 : (mask ? mask : 0xffffffff))));
...
::util::Status BcmSdkWrapper::AddL3RouteIpv6(int unit, int vrf,
  ...
  if (class_id > 0) {
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV6_UC_ROUTE_VRFs,
                                            CLASS_IDs, &min, &max));
    if ((class_id > static_cast<int>(max)) ||
        (class_id < static_cast<int>(min))) {
      return MAKE_ERROR(ERR_INVALID_PARAM)
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VRF_IDs, vrf));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, IPV6_UPPERs, ipv6_upper));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, IPV6_LOWERs, ipv6_lower));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, IPV6_UPPER_MASKs, ipv6_upper_mask));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, IPV6_LOWER_MASKs, ipv6_lower_mask));
  if (class_id > 0) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, CLASS_IDs, class_id));
  }
  RETURN_IF_BCM_ERROR(
...
::util::Status BcmSdkWrapper::ModifyL3HostIpv4(int unit, int vrf, uint32 ipv4,
  ...
  if (!entry_updated) {
    return MAKE_ERROR(ERR_ENTRY_NOT_FOUND)
           << "IPv4 L3 host " << PrintL3Host(host) << " not found on unit "
           << unit << ".";
  }
...
::util::Status BcmSdkWrapper::ModifyL3HostIpv6(int unit, int vrf,
  ...
  if (!entry_updated) {
    return MAKE_ERROR(ERR_ENTRY_NOT_FOUND)
           << "IPv6 L3 host " << PrintL3Host(host) << " not found on unit "
           << unit << ".";
  }
...
::util::StatusOr<int> BcmSdkWrapper::AddMyStationEntry(int unit, int priority,
  ...
  VLOG(1) << "Added dst MAC " << BcmMacToStr(mac) << "&&&"
          << BcmMacToStr(mac_mask) << " and VLAN " << vlan
          << " to my station TCAM with priority " << priority << " on unit "
          << unit << ".";
  return station_id;
}
...
::util::Status BcmSdkWrapper::DeleteMyStationEntry(int unit, int station_id) {
  ...
  const MyStationEntry* entry =
      FindIndexOrNull(*unit_to_my_stations, station_id);
  if (!entry) {
    return MAKE_ERROR(ERR_INVALID_PARAM)
...
::util::Status BcmSdkWrapper::AddL2Entry(int unit, int vlan, uint64 dst_mac,
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, L2_MC_GRP_IDs, l2_mcast_group_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, CLASS_IDs, class_id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, COPY_TO_CPUs, copy_to_cpu));
...
::util::Status BcmSdkWrapper::AddL2MulticastEntry(
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, COPY_TO_CPUs, copy_to_cpu || true));
...
::util::Status BcmSdkWrapper::DeleteVlanIfFound(int unit, int vlan) {
  ...
  if (entry_info.status == SHR_E_NOT_FOUND) {
    ...
    if (retval != SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
      return MAKE_ERROR(ERR_INTERNAL) << "Failed to delete VLAN " << vlan
                                      << " from unit " << unit << ".";
    }
...
::util::Status BcmSdkWrapper::ConfigureVlanBlock(int unit, int vlan,
  ...
  if (entry_info.status == SHR_E_NOT_FOUND) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "VLAN " << vlan << " does not exists on unit " << unit << ".";
  }
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, VLAN_PROFILE_IDs, &data));
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, VLAN_PROFILE_IDs, profile_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  if (entry_info.status == SHR_E_NOT_FOUND) {
    VLOG(1) << "VLAN profile " << static_cast<uint16>(profile_id)
            << " does not exist.";
    RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, VLAN_PROFILEs, &entry_hdl));
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, VLAN_PROFILE_IDs, profile_id));
  ...
  if (block_unknown_unicast ^ block_unknown_multicast) {
    LOG(WARNING)
        << "blocking does not differentiate between unknown uni and multicast";
  }
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, VLAN_PROFILEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, VLAN_PROFILE_IDs, profile_id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, L2_NON_UCAST_DROPs, block_broadcast));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, L2_MISS_DROPs,
                            block_unknown_multicast || block_unknown_unicast));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
...
::util::Status BcmSdkWrapper::ConfigureL2Learning(int unit, int vlan,
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  uint8 profile_id = data;
  VLOG(1) << "VLAN " << vlan << " has VLAN profile "
          << static_cast<uint16>(profile_id);
  if (profile_id == 0) {
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "VLAN " << vlan << " has no associated VLAN profile";
  }
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, VLAN_PROFILE_IDs, profile_id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, NO_LEARNINGs, disable_l2_learning));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
...
::util::Status BcmSdkWrapper::CreateKnetIntf(int unit, int vlan,
  ...
  // TODO(max): A valid VLAN (kDefaultVlan) is needed to get correct packet_in
  // into the ingress pipeline
  // But that adds VLAN tags to direct packet_outs. Maybe if there is a way to
  // stip outgoing VLAN tags.
  ...
  RETURN_IF_BCM_ERROR(
      bcmpkt_rx_register(unit, netif.id, 0, packet_receive_callback, NULL));
...
::util::Status BcmSdkWrapper::DestroyKnetIntf(int unit, int netif_id) {
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  RETURN_IF_BCM_ERROR(
      bcmpkt_rx_unregister(unit, netif_id, packet_receive_callback, 0));
...
::util::StatusOr<int> BcmSdkWrapper::CreateKnetFilter(int unit, int netif_id,
  ...
  switch (type) {
    case KnetFilterType::CATCH_NON_SFLOW_FP_MATCH:
      ...
      // BCMPKT_RX_REASON_CLEAR(filter.m_reason,
      // BCMPKT_RX_REASON_CPU_SFLOW_SRC);
      // BCMPKT_RX_REASON_CLEAR(filter.m_reason,
      // BCMPKT_RX_REASON_CPU_SFLOW_DST);
      // BCMPKT_RX_REASON_CLEAR(filter.m_reason,
      // BCMPKT_RX_REASON_CPU_SFLOW_FLEX);
      // BCMPKT_RX_REASON_CLEAR(filter.m_reason,
      // BCMPKT_RX_REASON_CPU_SFLOW_CPU_SFLOW_SRC);
      // BCMPKT_RX_REASON_CLEAR(filter.m_reason,
      // BCMPKT_RX_REASON_CPU_SFLOW_CPU_SFLOW_DST);
      // BCMPKT_RX_REASON_CLEAR(filter.m_reason,
      // BCMPKT_RX_REASON_CPU_SFLOW_CPU_SFLOW_FLEX);
...
::util::Status BcmSdkWrapper::SetRateLimit(
  ...
  for (const auto& e : rate_limit_config.per_cos_rate_limit_configs) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
        entry_hdl, TM_SCHEDULER_SHAPER_CPU_NODE_IDs, e.first));
    RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                           BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
    RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
...
::util::Status BcmSdkWrapper::GetKnetHeaderForDirectTx(int unit, int port,
  ...
  static_assert(sizeof(rcpu_header) == BCMPKT_RCPU_HDR_LEN,
                "sizeof(rcpu_header) != BCMPKT_RCPU_HDR_LEN");
  ...
  // RCPU headers.
  rcpu_header.ether_header.ether_type =
      htons(kRcpuVlanEthertype);  // bcmpkt_rcpu_hdr_s.tpid
  rcpu_header.vlan_tag.vlan_id = htons(kRcpuVlanId);
  rcpu_header.vlan_tag.type =
      htons(kRcpuEthertype);  // bcmpkt_rcpu_hdr_s.ethertype
  ...
  std::string s0(reinterpret_cast<const char*>(&rcpu_header),
                 sizeof(rcpu_header));
...
#define TXPMD_HEADER_TYPE_FROM_CPU 1
  ...
  RETURN_IF_BCM_ERROR(bcmpkt_txpmd_field_set(dev_type, meta, BCMPKT_TXPMD_START,
                                             TXPMD_START_IHEADER));
  RETURN_IF_BCM_ERROR(bcmpkt_txpmd_field_set(
      dev_type, meta, BCMPKT_TXPMD_HEADER_TYPE, TXPMD_HEADER_TYPE_FROM_CPU));
  RETURN_IF_BCM_ERROR(
      bcmpkt_txpmd_field_set(dev_type, meta, BCMPKT_TXPMD_UNICAST, 1));
  RETURN_IF_BCM_ERROR(bcmpkt_txpmd_field_set(
      dev_type, meta, BCMPKT_TXPMD_LOCAL_DEST_PORT, port));
  RETURN_IF_BCM_ERROR(
      bcmpkt_txpmd_field_set(dev_type, meta, BCMPKT_TXPMD_COS, cos));

  VLOG(2) << "txpmd: "
          << StringToHex(std::string(reinterpret_cast<const char*>(meta),
                                     sizeof(meta)));
...
static void dumpRxpmdHeaderRaw(const void* rxpmd) {
  ...
  for (int i = 0; i < BCMPKT_RXPMD_SIZE_WORDS; ++i) {
    VLOG(2) << "rxpmd [word " << absl::StrFormat("%02i", i)
            << "]: " << absl::StrFormat("%08x", words[i]);
  }
}
...
::util::Status BcmSdkWrapper::ParseKnetHeaderForRx(int unit,
  ...
  uint32 val;
  RETURN_IF_BCM_ERROR(bcmpkt_dev_type_get(unit, &dev_type));
  uint32* meta = reinterpret_cast<uint32*>(const_cast<char*>(&header[0]) +
                                           sizeof(RcpuHeader));
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_CPU_COS, &val));
  *cos = val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_SRC_PORT_NUM, &val));
  src_port = val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_QUEUE_NUM, &val));
  VLOG(2) << "queue_num " << val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_OUTER_VID, &val));
  VLOG(2) << "outer vid " << val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_MATCHED_RULE, &val));
  VLOG(2) << "matched rule " << val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_PKT_LENGTH, &val));
  VLOG(2) << "packet length " << val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_REASON_TYPE, &val));
  ...
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_HGI, &val));
  VLOG(2) << "hgi " << val;
  RETURN_IF_BCM_ERROR(bcmpkt_rxpmd_field_get(
      dev_type, meta, BCMPKT_RXPMD_TIMESTAMP_TYPE, &val));
  VLOG(2) << "timestamp type " << val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_TIMESTAMP, &val));
  VLOG(2) << "timestamp " << val;
  RETURN_IF_BCM_ERROR(
      bcmpkt_rxpmd_field_get(dev_type, meta, BCMPKT_RXPMD_TIMESTAMP_HI, &val));
  VLOG(2) << "timestamp hi " << val;
  dst_port =
      GetRxpmdField<uint8, 4, 7, 0>(meta);  // Reverse engineered dst port
...
::util::Status BcmSdkWrapper::SetAclControl(int unit,
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, PORT_FPs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, PORT_IDs, kCpuLogicalPort));
  if (acl_control.cpu_port_flags.apply) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
        entry_hdl, FP_VLANs, acl_control.cpu_port_flags.vfp_enable));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
        entry_hdl, FP_INGs, acl_control.cpu_port_flags.ifp_enable));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
        entry_hdl, FP_EGRs, acl_control.cpu_port_flags.efp_enable));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(
        entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  }
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
...
std::vector<std::pair<std::string, std::string>> GetPktTypeAndMode(
  ...
  std::sort(PortAnyPktIPV4L3SingleWide.begin(),
            PortAnyPktIPV4L3SingleWide.end());
  ...
  std::sort(PortAnyPktIPV4L3DoubleWide.begin(),
            PortAnyPktIPV4L3DoubleWide.end());
  ...
  std::sort(PortAnyPktIPV4L3AltDoubleWide.begin(),
            PortAnyPktIPV4L3AltDoubleWide.end());
  ...
  std::sort(PortAnyPktIPL3AnySingleWide.begin(),
            PortAnyPktIPL3AnySingleWide.end());
  ...
  std::sort(PortAnyPktNonIPL3SingleWide.begin(),
            PortAnyPktNonIPL3SingleWide.end());
  ...
  std::sort(PortAnyPktNonIPL3DoubleWide.begin(),
            PortAnyPktNonIPL3DoubleWide.end());
  ...
  std::sort(PortAnyPktNonIPL3AnySingleWide.begin(),
            PortAnyPktNonIPL3AnySingleWide.end());
  ...
  std::sort(PortAnyPktNonIPL3AltDoubleWide.begin(),
            PortAnyPktNonIPL3AltDoubleWide.end());
  ...
  std::sort(PortAnyPktIPV6L3SingleWide.begin(),
            PortAnyPktIPV6L3SingleWide.end());
  ...
  std::sort(PortAnyPktIPV6L3DoubleWide.begin(),
            PortAnyPktIPV6L3DoubleWide.end());
  ...
  std::sort(PortAnyPktIPV6L3AltDoubleWide.begin(),
            PortAnyPktIPV6L3AltDoubleWide.end());
  ...
  std::sort(PortHigigPktAnyL3AnyDoubleWide.begin(),
            PortHigigPktAnyL3AnyDoubleWide.end());
  ...
  std::sort(PortFrontPktAnyL3AnyDoubleWide.begin(),
            PortFrontPktAnyL3AnyDoubleWide.end());
  ...
  std::sort(PortLbkPktAnyL3AnyDoubleWide.begin(),
            PortLbkPktAnyL3AnyDoubleWide.end());
  ...
  if (std::includes(PortAnyPktAnyL2SingleWide.begin(),
                    PortAnyPktAnyL2SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L2_SINGLE_WIDEs, PORT_ANY_PACKET_ANYs));
  }

  if (std::includes(PortAnyPktIPV4L3SingleWide.begin(),
                    PortAnyPktIPV4L3SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_SINGLE_WIDEs, PORT_ANY_PACKET_IPV4s));
  }

  if (std::includes(PortAnyPktIPV6L3SingleWide.begin(),
                    PortAnyPktIPV6L3SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_SINGLE_WIDEs, PORT_ANY_PACKET_IPV6s));
  }

  if (std::includes(PortAnyPktNonIPL3SingleWide.begin(),
                    PortAnyPktNonIPL3SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_SINGLE_WIDEs, PORT_ANY_PACKET_NONIPs));
  }

  if (std::includes(PortAnyPktIPV4L3DoubleWide.begin(),
                    PortAnyPktIPV4L3DoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_DOUBLE_WIDEs, PORT_ANY_PACKET_IPV4s));
  }

  if (std::includes(PortAnyPktIPV6L3DoubleWide.begin(),
                    PortAnyPktIPV6L3DoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_DOUBLE_WIDEs, PORT_ANY_PACKET_IPV6s));
  }

  if (std::includes(PortAnyPktNonIPL3DoubleWide.begin(),
                    PortAnyPktNonIPL3DoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_DOUBLE_WIDEs, PORT_ANY_PACKET_NONIPs));
  }

  if (std::includes(PortAnyPktIPL3AnySingleWide.begin(),
                    PortAnyPktIPL3AnySingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ANY_SINGLE_WIDEs, PORT_ANY_PACKET_IPs));
  }

  if (std::includes(PortAnyPktNonIPL3AnySingleWide.begin(),
                    PortAnyPktNonIPL3AnySingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ANY_SINGLE_WIDEs, PORT_ANY_PACKET_NONIPs));
  }

  if (std::includes(PortHigigPktAnyL3AnyDoubleWide.begin(),
                    PortHigigPktAnyL3AnyDoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ANY_DOUBLE_WIDEs, PORT_HIGIG_PACKET_ANYs));
  }

  if (std::includes(PortFrontPktAnyL3AnyDoubleWide.begin(),
                    PortFrontPktAnyL3AnyDoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ANY_DOUBLE_WIDEs, PORT_LOOPBACK_PACKET_ANYs));
  }

  if (std::includes(PortLbkPktAnyL3AnyDoubleWide.begin(),
                    PortLbkPktAnyL3AnyDoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ANY_DOUBLE_WIDEs, PORT_FRONT_PACKET_ANYs));
  }

  if (std::includes(PortAnyPktIPV4L3AltDoubleWide.begin(),
                    PortAnyPktIPV4L3AltDoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ALT_DOUBLE_WIDEs, PORT_ANY_PACKET_IPV4s));
  }

  if (std::includes(PortAnyPktNonIPL3AltDoubleWide.begin(),
                    PortAnyPktNonIPL3AltDoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ALT_DOUBLE_WIDEs, PORT_ANY_PACKET_IPV6s));
  }

  if (std::includes(PortAnyPktIPV6L3AltDoubleWide.begin(),
                    PortAnyPktIPV6L3AltDoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(
        std::make_pair(L3_ALT_DOUBLE_WIDEs, PORT_ANY_PACKET_NONIPs));
  }

  return possible_combination;
}

std::pair<std::string, std::string> HalAclFieldToBcmRule(BcmAclStage stage,
                                                         BcmField::Type field) {
  // EFP specific field mappings.
  static auto* efp_field_map = new absl::flat_hash_map<
      BcmField::Type, std::pair<std::string, std::string>,
      ...
      {BcmField::IPV6_SRC_UPPER_64,
       {QUAL_SRC_IP6_HIGHs, QUAL_SRC_IP6_HIGH_MASKs}},
      {BcmField::IPV6_DST_UPPER_64,
       {QUAL_DST_IP6_HIGHs, QUAL_DST_IP6_HIGH_MASKs}},
      ...
      {BcmField::IP_PROTO_NEXT_HDR,
       {QUAL_IP_PROTOCOLs, QUAL_IP_PROTOCOL_MASKs}},
      {BcmField::L4_SRC, {QUAL_L4SRC_PORTs, QUAL_L4SRC_PORT_MASKs}},
      {BcmField::L4_DST, {QUAL_L4DST_PORTs, QUAL_L4DST_PORT_MASKs}},
      {BcmField::TCP_FLAGS,
       {QUAL_TCP_CONTROL_FLAGSs, QUAL_TCP_CONTROL_FLAGS_MASKs}},
      {BcmField::ICMP_TYPE_CODE,
       {QUAL_ICMP_TYPE_CODEs, QUAL_ICMP_TYPE_CODE_MASKs}},
  });

  // IFP specific field mappings.
  static auto* ifp_field_map = new absl::flat_hash_map<
      BcmField::Type, std::pair<std::string, std::string>,
      EnumHash<BcmField::Type>>({
      {BcmField::IN_PORT, {QUAL_INPORTs, QUAL_INPORT_MASKs}},
      ...
      // {BcmField::IPV6_DST, ??},
      {BcmField::IPV6_SRC_UPPER_64,
       {QUAL_SRC_IP6_UPPERs, QUAL_SRC_IP6_MASK_UPPERs}},
      {BcmField::IPV6_DST_UPPER_64,
       {QUAL_DST_IP6_UPPERs, QUAL_DST_IP6_MASK_UPPERs}},
      {BcmField::VRF, {QUAL_VRFs, QUAL_VRF_MASKs}},
      {BcmField::IP_DSCP_TRAF_CLASS, {QUAL_TOSs, QUAL_TOS_MASKs}},
      {BcmField::IP_TTL_HOP_LIMIT, {QUAL_TTLs, QUAL_TTL_MASKs}},
      {BcmField::IP_PROTO_NEXT_HDR,
      ...
      {BcmField::VFP_DST_CLASS_ID,
       {QUAL_FP_VLAN_CLASS0s, QUAL_FP_VLAN_CLASS0_MASKs}},
      {BcmField::L3_DST_CLASS_ID, {QUAL_L3DST_CLASSs, QUAL_L3DST_CLASS_MASKs}},
  });

  // VFP specific field mappings.
  static auto* vfp_field_map = new absl::flat_hash_map<
      BcmField::Type, std::pair<std::string, std::string>,
      EnumHash<BcmField::Type>>({
      {BcmField::IN_PORT, {QUAL_INPORTs, QUAL_INPORT_MASKs}},
      {BcmField::ETH_TYPE, {QUAL_ETHERTYPEs, QUAL_ETHERTYPE_MASKs}},
  ...
  std::pair<std::string, std::string> rule =
      std::make_pair(unknown_qual, unknown_qual);
  if (stage_map) {
...
std::string HalAclFieldToBcm(BcmAclStage stage, BcmField::Type field) {
  ...
  static auto* ifp_field_map =
      new absl::flat_hash_map<BcmField::Type, std::string,
          ...
          // {BcmField::IPV6_DST, ??}, // QUAL_DST_IP6_BITMAP_UPPER,
          // QUAL_DST_IP6_BITMAP_LOWER
  ...
  return gtl::FindWithDefault(*stage_map, field,
                              BcmField_Type_Name(BcmField::UNKNOWN));
}

::util::StatusOr<int> GetUniqueId(
    std::map<std::pair<BcmAclStage, int>, int>* table_ids, int id, int max) {
  int acl_id = 0;
  ...
  if (id != -1) {
    ...
    if (setOfNumbers.count(id) != 0) {
      return MAKE_ERROR(ERR_INTERNAL)
             << "Entry with table id " << id << " already exists.";
    } else {
      acl_id = id;
    }
  }
...
::util::Status CreateVfpGroup(int unit, int stage_id,
  ...
  for (const auto& field : table.fields()) {
    if (field.udf_chunk_id()) {
      ...
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
          entry_hdl, QUAL_UDF_CHUNKSs, index, &value, 1));
      continue;
    }
...
::util::Status CreateIfpGroup(int unit, int stage_id,
  ...
  for (const auto& field : table.fields()) {
    if (field.udf_chunk_id()) {
      ...
      uint64_t value = static_cast<uint64>(0xffff);
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
          entry_hdl, QUAL_UDF_CHUNKS_BITMAPs, index, &value, 1));
      continue;
    }
    std::string bcm_qual_field = HalAclFieldToBcm(table.stage(), field.type());
    std::string unknown_qual = BcmField_Type_Name(BcmField::UNKNOWN);
    ...
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(
        unit, FP_ING_GRP_TEMPLATEs, bcm_qual_field.c_str(), &min, &max));
    RETURN_IF_BCM_ERROR(
...
::util::Status CreateEfpGroup(int unit, int stage_id,
  ...
  for (const auto& field : table.fields()) {
    if (field.udf_chunk_id()) {
      return MAKE_ERROR(ERR_INTERNAL)
             << "UDF is not valid in " << BcmAclStage_Name(BCM_ACL_STAGE_EFP)
             << ".";
    }
    std::string bcm_qual_field = HalAclFieldToBcm(table.stage(), field.type());
    std::string unknown_qual = BcmField_Type_Name(BcmField::UNKNOWN);
  ...
  if (possibles.size() > 0) {
    for (const auto& p : possibles) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
          entry_hdl, PORT_PKT_TYPEs, p.second.c_str()));
      RETURN_IF_BCM_ERROR(
...
::util::Status CreateAclGroup(int unit, int id, BcmAclStage stage,
                              const BcmAclTable& table) {
...
::util::StatusOr<int> BcmSdkWrapper::CreateAclTable(int unit,
  ...
  ASSIGN_OR_RETURN(table_id,
                   GetUniqueId(table_ids, requested_table_id, maxEntries));
  // get next free slot
...
::util::Status BcmSdkWrapper::DestroyAclTable(int unit, int table_id) {
  ...
  if (SlotExists(group_ids, stage_id)) {
    rv = bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                   BCMLT_PRIORITY_NORMAL);
    if (rv == SHR_E_NONE) {
      entry_deleted = true;
    }
...
namespace {
::util::Status AddAclQualifier(
    int unit, const bcmlt_entry_handle_t& entry_hdl,
    const std::pair<std::string, std::string> field_pair,
    const BcmAclStage& stage, const BcmField& field) {
  uint64_t max_mask64;
  uint64_t min_mask64;
  uint64_t value64;
  uint32_t mask32;
  ...
  if (field_mask_name.length() > 0) {
    if (stage == BCM_ACL_STAGE_VFP) {
      RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_VLAN_RULE_TEMPLATEs,
                                              field_mask_name.c_str(),
                                              &min_mask64, &max_mask64));
    } else if (stage == BCM_ACL_STAGE_IFP) {
      RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_ING_RULE_TEMPLATEs,
                                              field_mask_name.c_str(),
                                              &min_mask64, &max_mask64));
    } else if (stage == BCM_ACL_STAGE_EFP) {
      RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_EGR_RULE_TEMPLATEs,
                                              field_mask_name.c_str(),
                                              &min_mask64, &max_mask64));
    } else {
      return MAKE_ERROR(ERR_INVALID_PARAM)
  ...
  switch (field.type()) {
    ...
    case BcmField::ETH_DST:
      ...
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_add(entry_hdl, field_name.c_str(), value64));
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
          entry_hdl, field_mask_name.c_str(), max_mask64));
      break;
    case BcmField::IP_TYPE: {
      ...
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
          entry_hdl, field_name.c_str(), ip_type.c_str()));
    } break;
    ...
    case BcmField::ICMP_TYPE_CODE:
      ...
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_add(entry_hdl, field_name.c_str(), value32));
      if (field.has_mask()) {
        mask32 = static_cast<uint32>(field.mask().u32());
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, field_mask_name.c_str(), mask32));
      } else {
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
            entry_hdl, field_mask_name.c_str(), max_mask64));
      }
  ...
  for (const auto& field : flow.fields()) {
    if (field.udf_chunk_id()) {
      ...
      if (field.value().b().size() != 2 ||
          (field.has_mask() &&
           field.mask().b().size() != 2)) {  // kUdfChunkSize: 2
        return MAKE_ERROR(ERR_INVALID_PARAM)
      ...
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
          entry_hdl, QUAL_UDF_CHUNKSs, index, &value64, 1));
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(
          entry_hdl, QUAL_UDF_CHUNKS_MASKs, index, &mask64, 1));
      continue;
    }
    std::pair<std::string, std::string> bcm_qual_field =
        HalAclFieldToBcmRule(flow.acl_stage(), field.type());
    if (bcm_qual_field.first == BcmField_Type_Name(BcmField::UNKNOWN)) {
      LOG(INFO) << "Qual: '" << field.ShortDebugString() << "' in "
                << BcmAclStage_Name(BCM_ACL_STAGE_VFP) << ".";
      return MAKE_ERROR()
             << "Attempted to translate unsupported BcmField::Type: "
             << BcmField::Type_Name(field.type()) << ".";
    }
    RETURN_IF_ERROR(AddAclQualifier(unit, entry_hdl, bcm_qual_field,
  ...
  for (const auto& field : flow.fields()) {
    if (field.udf_chunk_id()) {
      ...
      if (field.value().b().size() != BcmSdkWrapper::kUdfChunkSize ||
          (field.has_mask() &&
           field.mask().b().size() != BcmSdkWrapper::kUdfChunkSize)) {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to program flow with UDF chunk "
               << field.udf_chunk_id()
               << " with value or mask size not equal to "
    ...
    if (bcm_qual_field.first == BcmField_Type_Name(BcmField::UNKNOWN)) {
      LOG(INFO) << "Qual: '" << field.ShortDebugString() << "' in "
                << BcmAclStage_Name(BCM_ACL_STAGE_IFP) << ".";
      return MAKE_ERROR()
             << "Attempted to translate unsupported BcmField::Type: "
             << BcmField::Type_Name(field.type()) << ".";
    }
  ...
  switch (action.type()) {
    case BcmAction::DROP: {
      ...
      if (action.params().empty()) {  // No params, just drop
        if (stage == BCM_ACL_STAGE_VFP) {
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_DROPs, 1));
        } else if (stage == BCM_ACL_STAGE_IFP || stage == BCM_ACL_STAGE_EFP) {
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_G_DROPs, 1));
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_Y_DROPs, 1));
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_R_DROPs, 1));
        }
        break;
      }
    ...
    case BcmAction::OUTPUT_PORT: {
      ...
      if (stage == BCM_ACL_STAGE_IFP) {
        ...
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_REDIRECT_TO_PORTs, port));
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_REDIRECT_TO_MODULEs, 0));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << " in "
    ...
    case BcmAction::OUTPUT_TRUNK: {
      ...
      if (stage == BCM_ACL_STAGE_IFP) {
        ...
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_REDIRECT_TO_TRUNKs, trunk));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << " in "
    ...
    case BcmAction::OUTPUT_L3: {
      ...
      RETURN_IF_ERROR(VerifyAclActionParams(action, required, optional));
      if (stage == BCM_ACL_STAGE_IFP) {
        uint32 egress_intf_id =
            static_cast<uint32>(action.params(0).value().u32());
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
            entry_hdl, ACTION_SWITCH_TO_L3UCs, egress_intf_id));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << " in "
    ...
    case BcmAction::COPY_TO_CPU: {
        ...
        switch (param.type()) {
          case BcmAction::Param::QUEUE:
            if (stage == BCM_ACL_STAGE_VFP || stage == BCM_ACL_STAGE_IFP) {
              ...
              RETURN_IF_BCM_ERROR(
                  bcmlt_entry_field_add(entry_hdl, ACTION_NEW_CPU_COSs, queue));
            } else {
              return MAKE_ERROR(ERR_INVALID_PARAM)
                     << "Attempted to translate unsupported BcmAction::Type: "
                     << BcmAction::Type_Name(action.type()) << " in "
            ...
            switch (param.value().u32()) {
              case BcmAction::Param::GREEN:
                ...
                if (stage == BCM_ACL_STAGE_IFP) {
                  colorFound = true;
                  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
                      entry_hdl, ACTION_G_COPY_TO_CPUs, 1));
                } else {
                  return MAKE_ERROR(ERR_INVALID_PARAM)
                         << "Attempted to translate unsupported "
                            "BcmAction::Type: "
                         << BcmAction::Type_Name(action.type()) << " in "
                         << BcmAclStage_Name(stage) << ".";
                }
              ...
              case BcmAction::Param::YELLOW:
                ...
                if (stage == BCM_ACL_STAGE_IFP) {
                  colorFound = true;
                  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
                      entry_hdl, ACTION_Y_COPY_TO_CPUs, 1));
                } else {
                  return MAKE_ERROR(ERR_INVALID_PARAM)
                         << "Attempted to translate unsupported "
                            "BcmAction::Type: "
              ...
              case BcmAction::Param::RED:
                ...
                if (stage == BCM_ACL_STAGE_IFP) {
                  colorFound = true;
                  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
                      entry_hdl, ACTION_R_COPY_TO_CPUs, 1));
                } else {
                  return MAKE_ERROR(ERR_INVALID_PARAM)
                         << "Attempted to translate unsupported "
                            "BcmAction::Type: "
      ...
      if (!colorFound) {
        if (stage == BCM_ACL_STAGE_VFP) {
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_COPY_TO_CPUs, 1));
        } else if (stage == BCM_ACL_STAGE_IFP) {
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_G_COPY_TO_CPUs, 1));
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_Y_COPY_TO_CPUs, 1));
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_R_COPY_TO_CPUs, 1));
        } else {
          return MAKE_ERROR(ERR_INVALID_PARAM)
                 << "Attempted to translate unsupported BcmAction::Type: "
                 << BcmAction::Type_Name(action.type()) << " in "
    ...
    case BcmAction::CANCEL_COPY_TO_CPU: {
      ...
      if (action.params().empty()) {  // No params, just drop
        ...
        if (stage == BCM_ACL_STAGE_VFP) {
          RETURN_IF_BCM_ERROR(
              bcmlt_entry_field_add(entry_hdl, ACTION_COPY_TO_CPU_CANCELs, 1));
        } else if (stage == BCM_ACL_STAGE_IFP) {
          RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
              entry_hdl, ACTION_G_COPY_TO_CPU_CANCELs, 1));
          RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
              entry_hdl, ACTION_Y_COPY_TO_CPU_CANCELs, 1));
          RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
              entry_hdl, ACTION_R_COPY_TO_CPU_CANCELs, 1));
        } else {
          return MAKE_ERROR(ERR_INVALID_PARAM)
                 << "Attempted to translate unsupported BcmAction::Type: "
                 << BcmAction::Type_Name(action.type()) << " in "
    ...
    case BcmAction::SET_COLOR: {
      ...
      switch (color) {
        case BcmAction::Param::GREEN:
          if (stage == BCM_ACL_STAGE_VFP) {
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_COLORs, GREENs));
          } else if (stage == BCM_ACL_STAGE_IFP) {
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_Y_COLORs, GREENs));
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_R_COLORs, GREENs));
          } else {
            return MAKE_ERROR(ERR_INVALID_PARAM)
        ...
        case BcmAction::Param::YELLOW:
          if (stage == BCM_ACL_STAGE_VFP) {
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_COLORs, YELLOWs));
          } else if (stage == BCM_ACL_STAGE_IFP) {
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_R_COLORs, YELLOWs));
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_G_COLORs, YELLOWs));
          } else {
            return MAKE_ERROR(ERR_INVALID_PARAM)
                   << "Invalid color parameter for SET_COLOR action: " << color
                   << ".";
        ...
        case BcmAction::Param::RED:
          if (stage == BCM_ACL_STAGE_VFP) {
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_COLORs, REDs));
          } else if (stage == BCM_ACL_STAGE_IFP) {
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_Y_COLORs, REDs));
            RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(
                entry_hdl, ACTION_NEW_G_COLORs, REDs));
          } else {
            return MAKE_ERROR(ERR_INVALID_PARAM)
                   << "Invalid color parameter for SET_COLOR action: " << color
                   << ".";
    ...
    case BcmAction::SET_VRF: {
      ...
      if (stage == BCM_ACL_STAGE_VFP) {
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_VRF_SETs, vrf));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << ".";
    ...
    case BcmAction::SET_VFP_DST_CLASS_ID: {
      ...
      if (stage == BCM_ACL_STAGE_VFP) {
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
            entry_hdl, ACTION_DST_CLASS_ID_SETs, class_id));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << ".";
    ...
    case BcmAction::SET_IP_DSCP: {
      ...
      if (stage == BCM_ACL_STAGE_IFP || stage == BCM_ACL_STAGE_EFP) {
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_NEW_G_DSCPs, dscp));
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_NEW_Y_DSCPs, dscp));
        RETURN_IF_BCM_ERROR(
            bcmlt_entry_field_add(entry_hdl, ACTION_NEW_R_DSCPs, dscp));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << ".";
    ...
    case BcmAction::ADD_OUTER_VLAN: {
      ...
      if (stage == BCM_ACL_STAGE_EFP) {
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
            entry_hdl, ACTION_NEW_OUTER_VLANIDs, outer_vlan));
      } else if (stage == BCM_ACL_STAGE_VFP) {
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(
            entry_hdl, ACTION_ADD_OUTER_TAGs, outer_vlan));
      } else {
        return MAKE_ERROR(ERR_INVALID_PARAM)
               << "Attempted to translate unsupported BcmAction::Type: "
               << BcmAction::Type_Name(action.type()) << ".";
...
::util::Status CreateVfpPolicy(int unit, int policy_id,
  ...
  for (const auto& action : flow.actions()) {
    RETURN_IF_ERROR(
        AddAclAction(unit, entry_hdl, policy_id, flow.acl_stage(), action));
  }
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
...
::util::Status AddAclPolicer(int unit, int meter_id, int stage,
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, MIN_BURST_SIZE_KBITSs, committed_burst));
...
::util::Status CreateEfpEntry(int unit, int acl_id, int priority, int group_id,
                              int rule_id, int policy_id, int meter_id) {
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_EGR_ENTRYs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_EGR_ENTRY_IDs, acl_id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_EGR_GRP_TEMPLATE_IDs, group_id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_EGR_RULE_TEMPLATE_IDs, rule_id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_EGR_POLICY_TEMPLATE_IDs, policy_id));
...
::util::StatusOr<int> BcmSdkWrapper::InsertAclFlow(int unit,
  ...
  ASSIGN_OR_RETURN(
      found, FindAndReturnEntry(
                 group_ids, static_cast<int>(flow.bcm_acl_table_id()), &entry));
  if (!found) {
    return MAKE_ERROR(ERR_INVALID_PARAM)
  ...
  ASSIGN_OR_RETURN(policy_table_id,
                   GetUniqueId(policy_table_ids, -1, maxPolicies));
  ...
  if (flow.has_meter()) {
    if (stage == BCM_ACL_STAGE_IFP || stage == BCM_ACL_STAGE_EFP) {
      ...
      ASSIGN_OR_RETURN(meter_table_id,
                       GetUniqueId(meter_table_ids, -1, maxMeters));
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    RETURN_IF_ERROR(CreateVfpEntry(unit, acl_id, flow.priority(), group_id,
                                   rule_id, policy_id));
  } else if (stage == BCM_ACL_STAGE_IFP) {
    RETURN_IF_ERROR(CreateIfpEntry(unit, acl_id, flow.priority(), group_id,
                                   rule_id, policy_id, meter_id));
  } else {
    RETURN_IF_ERROR(CreateEfpEntry(unit, acl_id, flow.priority(), group_id,
                                   rule_id, policy_id, meter_id));
  }
...
::util::Status RemoveVfpRule(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, FP_VLAN_RULE_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_VLAN_RULE_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveVfpPolicy(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, FP_VLAN_POLICY_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_VLAN_POLICY_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveIfpRule(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, FP_ING_RULE_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_ING_RULE_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveIfpPolicy(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, FP_ING_POLICY_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_ING_POLICY_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveIfpMeter(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, METER_FP_ING_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, METER_FP_ING_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status DetachIfpMeter(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_ENTRY_IDs, id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, METER_FP_ING_TEMPLATE_IDs, 0));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveEfpRule(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, FP_EGR_RULE_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_EGR_RULE_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveEfpPolicy(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, FP_EGR_POLICY_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, FP_EGR_POLICY_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveEfpMeter(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_allocate(unit, METER_FP_EGR_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, METER_FP_EGR_TEMPLATE_IDs, id));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status DetachEfpMeter(int unit, int id) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_EGR_ENTRY_IDs, id));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, METER_FP_EGR_TEMPLATE_IDs, 0));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE,
                                                BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status RemoveVfpFlow(int unit, int rule_id, int policy_id,
                             int entry_id) {
...
::util::Status RemoveIfpFlow(int unit, int rule_id, int policy_id, int meter_id,
                             int entry_id) {
  if (meter_id > 0) {
...
::util::Status RemoveEfpFlow(int unit, int rule_id, int policy_id, int meter_id,
                             int entry_id) {
  if (meter_id > 0) {
...
::util::Status GetVfpEntry(int unit, int id, int* rule_id, int* policy_id) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, FP_VLAN_RULE_TEMPLATE_IDs, &data));
  *rule_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, FP_VLAN_POLICY_TEMPLATE_IDs, &data));
  *policy_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
}

::util::Status GetIfpEntry(int unit, int id, int* rule_id, int* policy_id,
                           int* meter_id) {
  uint64_t data;
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_ING_ENTRYs, &entry_hdl));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, FP_ING_RULE_TEMPLATE_IDs, &data));
  *rule_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, FP_ING_POLICY_TEMPLATE_IDs, &data));
  *policy_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, METER_FP_ING_TEMPLATE_IDs, &data));
  *meter_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
}

::util::Status GetEfpEntry(int unit, int id, int* rule_id, int* policy_id,
                           int* meter_id) {
  uint64_t data;
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_EGR_ENTRYs, &entry_hdl));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, FP_EGR_RULE_TEMPLATE_IDs, &data));
  *rule_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, FP_EGR_POLICY_TEMPLATE_IDs, &data));
  *policy_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_get(entry_hdl, METER_FP_EGR_TEMPLATE_IDs, &data));
  *meter_id = static_cast<int>(data);
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
}
...
::util::Status BcmSdkWrapper::ModifyAclFlow(int unit, int flow_id,
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    ...
    RETURN_IF_ERROR(RemoveVfpPolicy(unit, policy_id));
    RETURN_IF_ERROR(CreateVfpPolicy(unit, policy_id, flow));
    RETURN_IF_ERROR(CreateVfpEntry(unit, acl_entry_id, flow.priority(),
                                   group_id, rule_id, policy_id));
  } else if (stage == BCM_ACL_STAGE_IFP) {
    // TODO(BRCM): need to check if the flow is valid,
    // otherwise we should not delete existing flow
    RETURN_IF_ERROR(
        GetIfpEntry(unit, acl_entry_id, &rule_id, &policy_id, &meter_id));
    ...
    if (flow.has_meter()) {
      if (meter_id == 0) {
        ...
        ASSIGN_OR_RETURN(
            meter_id,
            GetFreeSlot(ifp_meter_ids, "TODO(BRCM): add proper error message"));
        if (!fp_meters) {
          return MAKE_ERROR(ERR_INTERNAL)
                 << "TODO(BRCM): Add proper error message";
        }
    ...
    RETURN_IF_ERROR(CreateIfpEntry(unit, acl_entry_id, flow.priority(),
                                   group_id, rule_id, policy_id, meter_id));
  } else if (stage == BCM_ACL_STAGE_EFP) {
    // TODO(BRCM): need to check if the flow is valid,
    // otherwise we should not delete existing flow
    RETURN_IF_ERROR(
        GetEfpEntry(unit, acl_entry_id, &rule_id, &policy_id, &meter_id));
    bool meter_deleted = false;
    if (meter_id > 0) {
    ...
    if (flow.has_meter()) {
      if (meter_id == 0) {
        ...
        ASSIGN_OR_RETURN(
            meter_id,
            GetFreeSlot(efp_meter_ids, "TODO(BRCM): add proper error message"));
        if (!fp_meters) {
          return MAKE_ERROR(ERR_INTERNAL)
                 << "TODO(BRCM): Add proper error message";
        }
    ...
    RETURN_IF_ERROR(CreateEfpEntry(unit, acl_entry_id, flow.priority(),
                                   group_id, rule_id, policy_id, meter_id));
  } else {
    return MAKE_ERROR(ERR_INVALID_PARAM)
...
::util::Status BcmSdkWrapper::RemoveAclFlow(int unit, int flow_id) {
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    ...
    RETURN_IF_ERROR(
        GetIfpEntry(unit, acl_entry_id, &rule_id, &policy_id, &meter_id));
    RETURN_IF_ERROR(
        RemoveIfpFlow(unit, rule_id, policy_id, meter_id, acl_entry_id));
    // remove ifp map
    rule_ids = gtl::FindOrNull(ifp_rule_ids_, unit);
    policy_ids = gtl::FindOrNull(ifp_policy_ids_, unit);
    meter_ids = gtl::FindOrNull(ifp_meter_ids_, unit);
    ...
    RETURN_IF_ERROR(
        GetEfpEntry(unit, acl_entry_id, &rule_id, &policy_id, &meter_id));
    RETURN_IF_ERROR(
        RemoveEfpFlow(unit, rule_id, policy_id, meter_id, acl_entry_id));
    // remove efp map
    rule_ids = gtl::FindOrNull(efp_rule_ids_, unit);
    policy_ids = gtl::FindOrNull(efp_policy_ids_, unit);
    meter_ids = gtl::FindOrNull(efp_meter_ids_, unit);
...
::util::Status BcmSdkWrapper::SetAclPolicer(int unit, int flow_id,
  ...
  if (stage == BCM_ACL_STAGE_IFP) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_ING_ENTRYs, &entry_hdl));
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, FP_ING_ENTRY_IDs, acl_entry_id));
    RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                           BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_get(entry_hdl, ENTRY_PRIORITYs, &data));
      priority = static_cast<int>(data);
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_get(entry_hdl, FP_ING_GRP_TEMPLATE_IDs, &data));
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_EGR_ENTRYs, &entry_hdl));
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, FP_EGR_ENTRY_IDs, acl_entry_id));
    RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                           BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
    if (entry_info.status == SHR_E_NONE) {
      ...
      priority = static_cast<int>(data);
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_get(entry_hdl, FP_EGR_GRP_TEMPLATE_IDs, &data));
      group_id = static_cast<int>(data);
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_get(entry_hdl, FP_EGR_RULE_TEMPLATE_IDs, &data));
      rule_id = static_cast<int>(data);
  ...
  if (meter_id > 0) {
    if (stage == BCM_ACL_STAGE_IFP) {
      ...
      RETURN_IF_ERROR(CreateIfpEntry(unit, acl_entry_id, priority, group_id,
                                     rule_id, policy_id, meter_id));
      ...
      RETURN_IF_ERROR(RemoveEfpEntry(unit, acl_entry_id));
      RETURN_IF_ERROR(AddAclPolicer(unit, meter_id, stage, meter));
      RETURN_IF_ERROR(CreateEfpEntry(unit, acl_entry_id, priority, group_id,
                                     rule_id, policy_id, meter_id));
    }
  } else {
    if (stage == BCM_ACL_STAGE_IFP) {
...
::util::Status BcmSdkWrapper::InsertPacketReplicationEntry(
  ...
  gtl::InsertOrDie(&multicast_group_id_to_replicas_,
                   mcast_entry.multicast_group_id(), ports);
...
::util::Status GetGroupDetails(int unit, int stage_id, int table_id,
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    ...
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "Invalid ACL stage " << BcmAclStage_Name(stage);
  }
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP,
                                         BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  if (entry_info.status == SHR_E_NONE) {
    ...
    for (int i = BcmField::UNKNOWN + 1; i <= BcmField::Type_MAX; ++i) {
      ...
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_get(entry_hdl, bcm_qual_field.c_str(), &value));
    ...
    value = 0;
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_get(entry_hdl, ENTRY_PRIORITYs, &value));
    ...
    if (stage == BCM_ACL_STAGE_VFP) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_get(
          entry_hdl, QUAL_UDF_CHUNKSs, 0, chunk_array, 16, &num_chunks));
    } else if (stage == BCM_ACL_STAGE_IFP) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_get(
          entry_hdl, QUAL_UDF_CHUNKS_BITMAPs, 0, chunk_array, 16, &num_chunks));
    }
    if (num_chunks) {
...
::util::Status BcmSdkWrapper::GetAclTable(int unit, int table_id,
  ...
  if ((stage == BCM_ACL_STAGE_VFP) || (stage == BCM_ACL_STAGE_IFP) ||
    ...
    return MAKE_ERROR(ERR_INTERNAL) << "ACL table with invalid pipeline stage: "
                                    << BcmAclStage_Name(stage) << ".";
  }
  return ::util::OkStatus();
}
...
::util::StatusOr<BcmChip::BcmChipType> BcmSdkWrapper::GetChipType(int unit) {
  ...
  if (it == unit_to_chip_type_.end()) {
    return MAKE_ERROR(ERR_INTERNAL)
           << "Unit " << unit
           << "  is not found in unit_to_chip_type_. Have you "
