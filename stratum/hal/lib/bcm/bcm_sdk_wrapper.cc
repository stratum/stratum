...
#include "bcmdrd_config.h"  // NOLINT
...
DEFINE_int64(port_counters_interval_in_usec, 100 * 1000, "Port counter interval in usecs.");
DEFINE_int32(max_num_linkscan_writers, 10, "Max number of linkscan event Writers supported.");
...
// TODO(unknown): synchronize access
...
extern "C" int bsl_out_hook(bsl_meta_t* meta, const char* format, va_list args) {
  ...
  if (pthread_self() == bcm_sdk_wrapper->GetDiagShellThreadId()) {
    ...
    // considered error.
    if (meta->severity <= BSL_WARN || meta->xtra == (BSL_LS_APPL_SHELL | BSL_DEBUG)) {
      LOG(ERROR) << "BSL error (" << file << ":" << line << ":" << func << "): " << msg;
    }
  }
...
extern "C" int knet_intf_remover(int unit, const bcmpkt_netif_t* netif, void* dummy) {
  return bcmpkt_netif_destroy(unit, netif->id);
}

// Callback for removing KNET filter.
extern "C" int knet_filter_remover(int unit, const bcmpkt_filter_t* filter, void* dummy) {
  return bcmpkt_filter_destroy(unit, filter->id);
}

// A callback function executed in BCM linkscan thread context.
extern "C" void sdk_linkscan_callback(bcmlt_table_notif_info_t* notify_info, void* user_data) {
  BcmSdkWrapper* bcm_sdk_wrapper = BcmSdkWrapper::GetSingleton();
  if (!bcm_sdk_wrapper) {
    LOG(ERROR) << "BcmSdkWrapper singleton instance is not initialized.";
    return;
  ...
  LOG(INFO) << "Unit: " << unit << " Port: " << port << " Link: " << (link ? "UP" : "DOWN") << ".";
...
extern "C" int packet_receive_callback(int unit, int netif_id, bcmpkt_packet_t* packet, void* arg) {
...
int GetFieldMinMaxValue(int unit, const char* table, const char* field, uint64_t* min, uint64_t* max) {
  ...
  rv = bcmlt_table_field_defs_get(unit, table, buf.size(), buf.data(), &actual_number);
...
std::string BcmMacToStr(const uint8 bcm_mac[6]) {
  ...
  std::string sep = "";
  for (int i = 0; i < 6; ++i) {
    buffer << sep << std::setfill('0') << std::setw(2) << std::hex << (bcm_mac[i] & 0xff);
    sep = ":";
  }
...
std::string PrintL3EgressIntf(l3_intf_object_t l3_intf_obj, int egress_intf_id) {
  uint8 mac[6];
  std::stringstream buffer;
...
FieldType GetDcbField(const void* dcb) {
  ...
  static_assert(start_bit - end_bit + 1 <= sizeof(FieldType) * 8, "Return type too small for the field");

  uint32 mask =
      ((static_cast<uint64>(1) << (start_bit + 1)) - 1) & ~((static_cast<uint64>(1) << end_bit) - 1);
...
//
template <int word, int field_start_bit, int field_end_bit, int value_start_bit, int value_end_bit>
bool SetSobSplitField(void* sob, uint32 value) {
  static_assert(value_start_bit >= value_end_bit, "Must have value_start_bit >= value_end_bit");
  static_assert(value_start_bit >= 0 && value_start_bit < 32, "Invalid value start bit");
  static_assert(value_end_bit >= 0 && value_end_bit < 32, "Invalid value end bit");
  static_assert(value_start_bit - value_end_bit == field_start_bit - field_end_bit, "Size must match");

  uint32 value_mask = ((static_cast<uint64>(1) << (value_start_bit + 1)) - 1) &
                      ~((static_cast<uint64>(1) << value_end_bit) - 1);

  return SetSobField<word, field_start_bit, field_end_bit>(sob, (value & value_mask) >> value_end_bit);
}
...
const typename Collection::key_type* FindIndexOrNull(const Collection& collection,
...
// TODO(unknown): Replace InUseMap with an array or vector, but not std::vector<bool>!
::util::StatusOr<int> GetFreeSlot(std::map<int, bool>* map, std::string ErrMsg) {
  for (auto& slot : *map) {
    if (!slot.second) {
      return slot.first;
    }
...
bool SlotExists(std::map<int, bool>* map, int index) { return gtl::ContainsKey(*map, index); }

int bcmlt_custom_entry_commit(bcmlt_entry_handle_t entry_hdl, bcmlt_opcode_t op,
...
::util::Status GetTableLimits(int unit, const char* table, int* min, int* max) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, TABLE_IDs, table));
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, INDEX_MAX_LIMITs, &table_max));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, INDEX_MIN_LIMITs, &table_min));
...
BcmSdkWrapper::BcmSdkWrapper(BcmDiagShell* bcm_diag_shell)
    : unit_to_chip_type_(),
      ...
      unit_to_my_station_max_limit_(),
      unit_to_my_station_min_limit_(),
      unit_to_l3_intf_max_limit_(),
      unit_to_l3_intf_min_limit_(),
      l3_interface_ids_(),
      l3_egress_interface_ids_(),
      l3_ecmp_egress_interface_ids_(),
      unit_to_fp_groups_max_limit_(),
      ifp_group_ids_(),
      efp_group_ids_(),
      vfp_group_ids_(),
      fp_group_ids_(),
      unit_to_fp_rules_max_limit_(),
      ifp_rule_ids_(),
      efp_rule_ids_(),
      vfp_rule_ids_(),
      fp_rule_ids_(),
      unit_to_fp_policy_max_limit_(),
      ifp_policy_ids_(),
      efp_policy_ids_(),
      vfp_policy_ids_(),
      fp_policy_ids_(),
      unit_to_fp_meter_max_limit_(),
      ifp_meter_ids_(),
      efp_meter_ids_(),
      fp_meter_ids_(),
      unit_to_fp_max_limit_(),
      ifp_acl_ids_(),
      efp_acl_ids_(),
      vfp_acl_ids_(),
      fp_acl_ids_(),
      unit_to_udf_chunk_ids_(),
      unit_to_chunk_ids_(),
...
::util::Status BcmSdkWrapper::InitializeSdk(const std::string& config_file_path,
  ...
  if ((ndev = bcma_sys_conf_drd_init(isc)) < 0) {
    return MAKE_ERROR(ERR_INTERNAL) << "Could not find any supported device.";
  }
...
::util::Status BcmSdkWrapper::FindUnit(int unit, int pci_bus, int pci_slot, BcmChip::BcmChipType chip_type) {
  int num_devices = 0;
  int u = 0;
  ...
  if (!probed) {
    return MAKE_ERROR(ERR_INTERNAL) << "BDE not initialized yet. Call InitializeSdk() first.";
  }
    ...
    for (u = 0; u < BCMDRD_CONFIG_MAX_UNITS; u++) {
      if (bcmdrd_dev_exists(u)) {
        ...
        if (dev != NULL) {
          ...
          if ((di->device_id == dev->id.device_id) && (di->vendor_id == dev->id.vendor_id)) {
            ...
            if (u != unit) {
              return MAKE_ERROR(ERR_INTERNAL) << "Unit " << unit << " was not assigned to SOC device "
                                              << dev->name << " found on PCI bus " << pci_bus << ", PCI slot "
                                              << pci_slot << ". The device handle for this SOC device (" << u
                                              << ") does not match the unit number.";
            }
            LOG(INFO) << "Unit " << unit << " is assigned to SOC device " << dev->name << " found on PCI bus "
                      << pci_bus << ", PCI slot " << pci_slot << ".";
            return ::util::OkStatus();
          }
        }
      }
  ...
  return MAKE_ERROR(ERR_INTERNAL) << "Could not find any SOC device on PCI bus " << pci_bus << ", PCI slot "
                                  << pci_slot << ".";
}

::util::Status BcmSdkWrapper::InitializeUnit(int unit, bool warm_boot) {
  uint64_t all_ports_no_cpu_bitmap[3] = {0xFFFFFFFFFFFFFFFeULL, kuint64max, 0};
  int rv;
  ...
  // SOC device init.
  {
    ...
    if (!unit_to_soc_device_.count(unit)) {
      return MAKE_ERROR(ERR_INTERNAL) << "Unit " << unit << " has not been assigned to any SOC device.";
    }
    ...
    while ((rv = bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_TRAVERSE, BCMLT_PRIORITY_NORMAL)) == SHR_E_NONE) {
      if (bcmlt_entry_info_get(entry_hdl, &entry_info) != SHR_E_NONE || entry_info.status != SHR_E_NONE) {
        break;
      }
      uint64_t l_port;
      ...
      if (bcmlt_entry_field_get(entry_hdl, PC_PHYS_PORT_IDs, &physical_device_port) != SHR_E_NONE) {
        break;
      }
    ...
    for (const auto& p : configured_ports) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PC_PHYS_PORT_IDs, p.second));
      RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, PC_PM_IDs, &port_macro_id));
  ...
  // IFP - group
  RETURN_IF_ERROR(GetTableLimits(unit, FP_ING_GRP_TEMPLATEs, &table_min, &table_max));
  InUseMap ifp_groups;
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // VFP - group
  InUseMap vfp_groups;
  RETURN_IF_ERROR(GetTableLimits(unit, FP_VLAN_GRP_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // EFP - group
  InUseMap efp_groups;
  RETURN_IF_ERROR(GetTableLimits(unit, FP_EGR_GRP_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // IFP - rules
  InUseMap ifp_rules;
  RETURN_IF_ERROR(GetTableLimits(unit, FP_ING_RULE_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // VFP - rules
  InUseMap vfp_rules;
  RETURN_IF_ERROR(GetTableLimits(unit, FP_VLAN_RULE_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // EFP - rules
  InUseMap efp_rules;
  RETURN_IF_ERROR(GetTableLimits(unit, FP_EGR_RULE_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_ERROR(GetTableLimits(unit, FP_ING_POLICY_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_ERROR(GetTableLimits(unit, FP_VLAN_POLICY_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  RETURN_IF_ERROR(GetTableLimits(unit, FP_EGR_POLICY_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // IFP - Meters
  InUseMap ifp_meters;
  RETURN_IF_ERROR(GetTableLimits(unit, METER_FP_ING_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  // EFP - Meters
  InUseMap efp_meters;
  RETURN_IF_ERROR(GetTableLimits(unit, METER_FP_EGR_TEMPLATEs, &table_min, &table_max));
  for (auto i = table_min; i <= table_max; i++) {
  ...
  for (const auto& p : configured_ports) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_add(entry_hdl, STATEs, p.first, vlan_stg_str, 1));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, VLAN_ING_TAG_ACTION_PROFILEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_ING_TAG_ACTION_PROFILE_IDs, 1));
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_array_add(entry_hdl, EGR_MEMBER_PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_array_add(entry_hdl, ING_MEMBER_PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_array_add(entry_hdl, UNTAGGED_MEMBER_PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
  ...
  for (const auto& p : configured_ports) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, PORT_TYPEs, ETHERNETs));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, V4L3s, 1));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, V6L3s, 1));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_PKT_CONTROL_IDs, 1));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  }
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  ...
  for (const auto& p : configured_ports) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, p.first));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, PASS_ON_OUTER_TPID_MATCHs, 0,
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, PORTSs, 0, all_ports_no_cpu_bitmap, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, INTERVALs, FLAGS_port_counters_interval_in_usec));
  ...
  LOG(INFO) << "Unit " << unit << " initialized successfully (warm_boot: " << (warm_boot ? "YES" : "NO")
            << ").";
...
::util::Status BcmSdkWrapper::ShutdownUnit(int unit) {
  ...
  if (isc == NULL) {
    return MAKE_ERROR(ERR_INTERNAL) << "System configuration structure is not initialized.";
  }
  ...
  if (SHR_FAILURE(rv)) {
    return MAKE_ERROR(ERR_INTERNAL) << "Failed to shutdown the SDK System Manager.";
  }

  // Clean up HA file for the unit
  if (!bcmdrd_dev_exists(unit)) {
    return MAKE_ERROR(ERR_INTERNAL) << "Unit " << unit << "  is not found.";
  } else {
  ...
  if (SHR_FAILURE(rv)) {
    return MAKE_ERROR(ERR_INTERNAL) << "Failed to cleanup system log.";
  }
...
::util::Status BcmSdkWrapper::ShutdownAllUnits() {
  ...
  std::transform(unit_to_soc_device_.begin(), unit_to_soc_device_.end(), std::back_inserter(units),
                 [](decltype(unit_to_soc_device_)::value_type const& p) { return p.first; });
  for (int unit : units) {
    APPEND_STATUS_IF_ERROR(status, ShutdownUnit(unit));
  }
  gtl::STLDeleteValues(&unit_to_soc_device_);  // if some entries were not deleted
...
::util::Status BcmSdkWrapper::InitializePort(int unit, int port) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAX_FRAME_SIZEs, kDefaultMaxFrameSize));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  // Linkscan mode
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, port));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, LINKSCAN_MODEs, SOFTWAREs));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_add(entry_hdl, STATEs, port, &block, 1));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
  } else {
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
...
::util::Status BcmSdkWrapper::SetPortOptions(int unit, int port, const BcmPortOptions& options) {
  const char* block = "BLOCK";
  ...
  if (options.enabled()) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, port));
    RETURN_IF_BCM_ERROR(
        bcmlt_entry_field_add(entry_hdl, ENABLEs, (options.enabled() == TRI_STATE_TRUE ? 1 : 0)));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  }
  // STP State
  if (options.blocked()) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_add(
        entry_hdl, STATEs, port, (options.blocked() == TRI_STATE_TRUE ? &block : &forward), 1));
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
    } else {
      RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  ...
  if (options.max_frame_size() > 0) {
    ...
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, PC_PORTs, MAX_FRAME_SIZEs, &min, &max));
    if ((options.max_frame_size() > static_cast<int>(max)) ||
        (options.max_frame_size() < static_cast<int>(min))) {
      return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid mtu (" << options.max_frame_size()
                                           << "), valid mtu range is " << static_cast<int>(min) << " - "
                                           << static_cast<int>(max) << ".";
    }
    RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, PC_PORTs, &entry_hdl));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, port));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAX_FRAME_SIZEs, options.max_frame_size()));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  }
  // Linkscan
  if (options.linkscan_mode()) {
    ...
    switch (options.linkscan_mode()) {
      ...
      case BcmPortOptions::LINKSCAN_MODE_HW:
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, LINKSCAN_MODEs, HARDWAREs));
        break;
      default:
        RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, LINKSCAN_MODEs, NO_SCANs));
        break;
    }
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
...
::util::Status BcmSdkWrapper::GetPortOptions(int unit, int port, BcmPortOptions* options) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  uint64_t port_macro_id;
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_get(entry_hdl, LINKSCAN_MODEs, &linkscan_mode));
  ...
  if (entry_info.status == SHR_E_NONE) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, MAX_FRAME_SIZEs, &max_frame_size));
    options->set_max_frame_size(max_frame_size);
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, PC_PHYS_PORT_IDs, &physical_device_port));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_get(entry_hdl, OPMODEs, &op_mode));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, PC_PHYS_PORTs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PC_PHYS_PORT_IDs, physical_device_port));
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, PC_PM_IDs, &port_macro_id));
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PC_PM_IDs, port_macro_id));
    RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_array_get(entry_hdl, LANE_MAPs, 0, &lanemap_array, 1, &actual_count));
      ...
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_array_get(entry_hdl, SPEED_MAXs, 0, &speed_array, 1, &actual_count));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_STG_IDs, kDefaultVlanStgId));
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_symbol_get(entry_hdl, STATEs, 0, sym_res, 140, &actual_count));
...
::util::Status BcmSdkWrapper::GetPortCounters(int unit, int port, PortCounters* pc) {
  // Check if unit is valid
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  // Check if port is valid
  RETURN_IF_BCM_ERROR(CheckIfPortExists(unit, port)) << "Port " << port << " does not exit on unit " << unit
                                                     << ".";
...
::util::Status BcmSdkWrapper::StartLinkscan(int unit) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_table_subscribe(unit, LM_LINK_STATEs, &sdk_linkscan_callback, NULL));

  absl::WriterMutexLock l(&data_lock_);
  // // Get logical ports for this unit
  auto logical_ports_map = gtl::FindOrNull(unit_to_logical_ports_, unit);
  CHECK_RETURN_IF_FALSE(logical_ports_map != nullptr) << "Logical ports are not identified on the Unit "
                                                      << unit << ".";
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, SCAN_INTERVALs, FLAGS_linkscan_interval_in_usec));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, SCAN_ENABLEs, &enable));
    if (enable == 0) {
...
::util::StatusOr<int> BcmSdkWrapper::RegisterLinkscanEventWriter(
  ...
  CHECK_RETURN_IF_FALSE(linkscan_event_writers_.size() < static_cast<size_t>(FLAGS_max_num_linkscan_writers))
      << "Can only support " << FLAGS_max_num_linkscan_writers << " linkscan event Writers.";

  // Find the next available ID for the Writer.
  int next_id = kInvalidWriterId;
  for (int id = 1; id <= static_cast<int>(linkscan_event_writers_.size()) + 1; ++id) {
    auto it = std::find_if(linkscan_event_writers_.begin(), linkscan_event_writers_.end(),
...
::util::StatusOr<BcmPortOptions::LinkscanMode> BcmSdkWrapper::GetPortLinkscanMode(int unit, int port) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_get(entry_hdl, LINKSCAN_MODEs, &linkscan_str));
...
::util::Status BcmSdkWrapper::SetMtu(int unit, int mtu) {
  ...
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, PC_PORTs, MAX_FRAME_SIZEs, &min, &max));
  if ((mtu > static_cast<int>(max)) || (mtu < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid mtu (" << mtu << "), valid mtu range is "
...
::util::StatusOr<int> BcmSdkWrapper::FindOrCreateL3RouterIntf(int unit, uint64 router_mac, int vlan) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  uint64_t max;
  uint64_t min;
  ...
  if ((vlan > static_cast<int>(max)) || (vlan < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vlan (" << vlan << "), valid vlan range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }

  L3Interfaces entry(router_mac, vlan);
  auto unit_to_l3_intf = gtl::FindOrNull(l3_interface_ids_, unit);
  CHECK_RETURN_IF_FALSE(unit_to_l3_intf != nullptr) << "Unit " << unit
                                                    << "  is not found in l3_interface_ids. Have you "
  ...
  if (unit_to_l3_intf->count(entry)) {
    ...
    if (it != unit_to_l3_intf->end()) {
      ...
      VLOG(1) << "L3 intf " << PrintL3RouterIntf(l3_interface) << " already exists on unit " << unit << ".";
      return it->second;
    }
  }
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, L3_EIF_IDs, l3a_intf_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, (vlan > 0 ? vlan : 1)));
  ...
  VLOG(1) << "Created a new L3 router intf: " << PrintL3RouterIntf(l3_interface) << " on unit " << unit
          << ".";

  // update mtu
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L3_UC_MTUs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, (vlan > 0 ? vlan : kDefaultVlan)));
  RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, (vlan > 0 ? vlan : kDefaultVlan)));
...
::util::Status BcmSdkWrapper::DeleteL3RouterIntf(int unit, int router_intf_id) {
  ...
  if (!entry) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Router ID " << router_intf_id << " not found.";
  }
  // delete entry
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L3_EIFs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, L3_EIF_IDs, router_intf_id));
  ...
  VLOG(1) << "Router intf with ID " << router_intf_id << " deleted on unit " << unit << ".";
  return ::util::OkStatus();
}
...
::util::StatusOr<int> BcmSdkWrapper::FindOrCreateL3CpuEgressIntf(int unit) {
  ...
  CHECK_RETURN_IF_FALSE(l3_intfs != nullptr) << "Unit " << unit
                                             << " not initialized yet. Call InitializeUnit first.";
  // get next free slot
  ASSIGN_OR_RETURN(egress_intf_id, GetFreeSlot(l3_intfs, "L3 Port egress interface table is full."));
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L3_UC_NHOPs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, NHOP_IDs, egress_intf_id));
  ...
  VLOG(1) << "Created a new L3 CPU egress intf: " << PrintL3EgressIntf(l3_intf_o, egress_intf_id)
          << " on unit " << unit << ".";
  return egress_intf_id;
}

::util::StatusOr<int> BcmSdkWrapper::FindOrCreateL3PortEgressIntf(int unit, uint64 nexthop_mac, int port,
  ...
  // Check if the unit is valid
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_UC_NHOPs, VLAN_IDs, &min, &max));
  if ((vlan > static_cast<int>(max)) || (vlan < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vlan (" << vlan << "), valid vlan range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  ...
  VLOG(1) << "Created a new L3 port egress intf: " << PrintL3EgressIntf(l3_intf_o, egress_intf_id)
          << " on unit " << unit << ".";
  return egress_intf_id;
}

::util::StatusOr<int> BcmSdkWrapper::FindOrCreateL3TrunkEgressIntf(int unit, uint64 nexthop_mac, int trunk,
                                                                   int vlan, int router_intf_id) {
  bcmlt_entry_handle_t entry_hdl;
  int egress_intf_id = 0;
  bool found;
  ...
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_UC_NHOPs, TRUNK_IDs, &min, &max));
  if ((trunk > static_cast<int>(max)) || (trunk < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid trunk (" << trunk << "), valid trunk range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  InUseMap* l3_intfs = gtl::FindOrNull(l3_egress_interface_ids_, unit);
  auto unit_to_l3_intf = gtl::FindOrNull(l3_interface_ids_, unit);
  CHECK_RETURN_IF_FALSE(l3_intfs != nullptr && unit_to_l3_intf != nullptr)
      << "Unit " << unit << " not initialized yet. Call InitializeUnit first.";
  // get next free slot
  ASSIGN_OR_RETURN(egress_intf_id, GetFreeSlot(l3_intfs, "L3 Trunk egress interface table is full."));
  ...
  VLOG(1) << "Created a new L3 trunk egress intf: " << PrintL3EgressIntf(l3_intf_o, egress_intf_id)
          << " on unit " << unit << ".";
  return egress_intf_id;
}
...
::util::StatusOr<int> BcmSdkWrapper::FindOrCreateL3DropIntf(int unit) {
  ...
  ConsumeSlot(l3_intfs, egress_intf_id);
  l3_intf_object_t l3_intf_o = {0, 0x0, 1, 0, 0};
  VLOG(1) << "Created a new L3 drop egress intf: " << PrintL3EgressIntf(l3_intf_o, egress_intf_id)
          << " on unit " << unit << ".";
  return egress_intf_id;
}

::util::Status BcmSdkWrapper::ModifyL3CpuEgressIntf(int unit, int egress_intf_id) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  ...
  CHECK_RETURN_IF_FALSE(l3_egress_intf != nullptr) << "Unit " << unit
                                                   << " not initialized yet. Call InitializeUnit first.";
  // Check if egress interface is valid
  it = l3_egress_intf->find(egress_intf_id);
  if (it != l3_egress_intf->end()) {
    if (!it->second) {
      return MAKE_ERROR(ERR_INTERNAL) << "L3 Egress interface " << egress_intf_id << " is not created.";
    }
  } else {
    return MAKE_ERROR(ERR_INTERNAL) << "Invalid L3 Egress interface " << egress_intf_id << ".";
  }
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, L3_EIF_IDs, &l3_eif_id));
  ...
  VLOG(1) << "Modified L3 CPU egress intf while keeping its ID the same: "
          << PrintL3EgressIntf(l3_intf_o, egress_intf_id) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::ModifyL3PortEgressIntf(int unit, int egress_intf_id, uint64 nexthop_mac,
...
::util::Status BcmSdkWrapper::ModifyL3TrunkEgressIntf(int unit, int egress_intf_id, uint64 nexthop_mac,
...
::util::Status BcmSdkWrapper::DeleteL3EgressIntf(int unit, int egress_intf_id) {
  ...
  VLOG(1) << "Egress intf with ID " << egress_intf_id << " deleted on unit " << unit << ".";
  return ::util::OkStatus();
}

::util::StatusOr<int> BcmSdkWrapper::FindRouterIntfFromEgressIntf(int unit, int egress_intf_id) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  InUseMap::iterator it;
  uint64_t l3_eif_id;
  ...
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, L3_EIF_IDs, &l3_eif_id));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, MAC_DAs, &mac_da));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_get(entry_hdl, COPY_TO_CPUs, &copy_to_cpu));
...
::util::StatusOr<int> BcmSdkWrapper::FindOrCreateEcmpEgressIntf(int unit,
  ...
  CHECK_RETURN_IF_FALSE(ecmp_intfs != nullptr) << "Unit " << unit
                                               << " not initialized yet. Call InitializeUnit first.";
  // get next free slot
  ASSIGN_OR_RETURN(ecmp_intf_id, GetFreeSlot(ecmp_intfs, "ECMP egress interface table is full."));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, NUM_PATHSs, members_count));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, NHOP_IDs, 0, members_array, members_count));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  // consume slot
  ...
  VLOG(1) << "New ECMP group with ID " << ecmp_intf_id
          ...
          << PrintArray(members_array, members_count, ", ") << " on unit " << unit << ".";
  return ecmp_intf_id;
}

::util::Status BcmSdkWrapper::ModifyEcmpEgressIntf(int unit, int egress_intf_id,
                                                   const std::vector<int>& member_ids) {
  bcmlt_entry_handle_t entry_hdl;
  InUseMap::iterator it;
  // Check if the unit is valid
  ...
  if (it != ecmp_intfs->end()) {
    if (!it->second) {
      return MAKE_ERROR(ERR_INTERNAL) << "ECMP egress interface " << egress_intf_id << " is not created.";
    }
  } else {
    return MAKE_ERROR(ERR_INTERNAL) << "Invalid ECMP egress interface " << egress_intf_id << ".";
  }
...
::util::Status BcmSdkWrapper::DeleteEcmpEgressIntf(int unit, int egress_intf_id) {
  bcmlt_entry_handle_t entry_hdl;
  InUseMap::iterator it;
  // Check if the unit is valid
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  ...
  VLOG(1) << "ECMP group with ID " << egress_intf_id << " deleted on unit " << unit << ".";
  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::AddL3RouteIpv4(int unit, int vrf, uint32 subnet, uint32 mask, int class_id,
  ...
  // Check if the unit is valid
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV4_UC_ROUTE_VRFs, VRF_IDs, &min, &max));
  if ((vrf > static_cast<int>(max)) || (vrf < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vrf (" << vrf << "), valid vrf range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  if (class_id > 0) {
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV4_UC_ROUTE_VRFs, CLASS_IDs, &min, &max));
    if ((class_id > static_cast<int>(max)) || (class_id < static_cast<int>(min))) {
      return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid class_id (" << class_id
                                           << "), valid class_id range is " << static_cast<int>(min) << " - "
                                           << static_cast<int>(max) << ".";
    }
  }
  InUseMap* l3_egress_intf = gtl::FindOrNull(l3_egress_interface_ids_, unit);
  ...
  if (it != l3_egress_intf->end()) {
    if (!it->second) {
      return MAKE_ERROR(ERR_INVALID_PARAM) << "L3 Egress interface " << egress_intf_id << " is not created.";
    }
  } else {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid L3 Egress interface " << egress_intf_id << ".";
  }
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L3_IPV4_UC_ROUTE_VRFs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VRF_IDs, vrf));
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, IPV4_MASKs, (!subnet ? 0 : (mask ? mask : 0xffffffff))));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, ECMP_NHOPs, is_intf_multipath));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  if (rv == SHR_E_EXISTS) {
    return MAKE_ERROR(ERR_ENTRY_EXISTS) << "IPv4 L3 LPM route " << PrintL3Route(route)
                                        << " already exists on unit " << unit << ".";
  }
  VLOG(1) << "Added IPv4 L3 LPM route " << PrintL3Route(route) << " on unit " << unit << ".";
  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::AddL3RouteIpv6(int unit, int vrf, const std::string& subnet,
                                             const std::string& mask, int class_id, int egress_intf_id,
  ...
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV6_UC_ROUTE_VRFs, VRF_IDs, &min, &max));
  if ((vrf > static_cast<int>(max)) || (vrf < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vrf (" << vrf << "), valid vrf range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  if (class_id > 0) {
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV6_UC_ROUTE_VRFs, CLASS_IDs, &min, &max));
    if ((class_id > static_cast<int>(max)) || (class_id < static_cast<int>(min))) {
      return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid class_id (" << class_id
                                           << "), valid class_id range is " << static_cast<int>(min) << " - "
                                           << static_cast<int>(max) << ".";
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L3_IPV6_UC_ROUTE_VRFs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VRF_IDs, vrf));
  ...
  VLOG(1) << "Added IPv6 L3 LPM route " << PrintL3Route(route) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::AddL3HostIpv4(int unit, int vrf, uint32 ipv4, int class_id,
                                            int egress_intf_id) {
  bcmlt_entry_handle_t entry_hdl;
  uint64_t max;
  uint64_t min;
  ...
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV4_UC_HOSTs, NHOP_IDs, &min, &max));
  if ((egress_intf_id > static_cast<int>(max)) || (egress_intf_id < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid egress interface (" << egress_intf_id
                                         << "), valid next hop id range is " << static_cast<int>(min) << " - "
                                         << static_cast<int>(max) << ".";
  }

  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV4_UC_HOSTs, VRF_IDs, &min, &max));
  if ((vrf > static_cast<int>(max)) || (vrf < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vrf (" << vrf << "), valid vrf range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  ...
  VLOG(1) << "Added IPv4 L3 host route " << PrintL3Host(host) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::AddL3HostIpv6(int unit, int vrf, const std::string& ipv6, int class_id,
                                            int egress_intf_id) {
  bcmlt_entry_handle_t entry_hdl;
  uint64_t max;
  uint64_t min;
  ...
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV6_UC_HOSTs, VRF_IDs, &min, &max));
  if ((vrf > static_cast<int>(max)) || (vrf < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vrf (" << vrf << "), valid vrf range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }

  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L3_IPV6_UC_HOSTs, NHOP_IDs, &min, &max));
  if ((egress_intf_id > static_cast<int>(max)) || (egress_intf_id < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid egress interface (" << egress_intf_id
                                         << "), valid next hop id range is " << static_cast<int>(min) << " - "
                                         << static_cast<int>(max) << ".";
  ...
  VLOG(1) << "Added IPv6 L3 host route " << PrintL3Host(host) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::ModifyL3RouteIpv4(int unit, int vrf, uint32 subnet, uint32 mask, int class_id,
                                                int egress_intf_id, bool is_intf_multipath) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  uint64_t max;
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  if (!entry_updated) {
    return MAKE_ERROR(ERR_ENTRY_NOT_FOUND) << "IPv4 L3 LPM route " << PrintL3Route(route)
                                           << " not found on unit " << unit << ".";
  }
  VLOG(1) << "Modify IPv4 L3 LPM route " << PrintL3Route(route) << " on unit " << unit << ".";
  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::ModifyL3RouteIpv6(int unit, int vrf, const std::string& subnet,
                                                const std::string& mask, int class_id, int egress_intf_id,
                                                bool is_intf_multipath) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  if (!entry_updated) {
    return MAKE_ERROR(ERR_ENTRY_NOT_FOUND) << "IPv6 L3 LPM route " << PrintL3Route(route)
                                           << " not found on unit " << unit << ".";
  }
  VLOG(1) << "Modify IPv6 L3 LPM route " << PrintL3Route(route) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::ModifyL3HostIpv4(int unit, int vrf, uint32 ipv4, int class_id,
                                               int egress_intf_id) {
  uint64_t max;
  uint64_t min;
  bool entry_updated = false;
  ...
  if (!entry_updated) {
    return MAKE_ERROR(ERR_ENTRY_NOT_FOUND) << "IPv4 L3 host " << PrintL3Host(host) << " not found on unit "
                                           << unit << ".";
  }
  VLOG(1) << "Modify IPv4 L3 host route " << PrintL3Host(host) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::ModifyL3HostIpv6(int unit, int vrf, const std::string& ipv6, int class_id,
                                               int egress_intf_id) {
  uint64_t max;
  uint64_t min;
  bool entry_updated = false;
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  if (!entry_updated) {
    return MAKE_ERROR(ERR_ENTRY_NOT_FOUND) << "IPv6 L3 host " << PrintL3Host(host) << " not found on unit "
                                           << unit << ".";
  }

  VLOG(1) << "Modify IPv6 L3 host route " << PrintL3Host(host) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::DeleteL3RouteIpv4(int unit, int vrf, uint32 subnet, uint32 mask) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  uint64_t max;
  uint64_t min;
  ...
  VLOG(1) << "Deleted IPv4 L3 LPM route " << PrintL3Route(route) << " on unit " << unit << ".";
  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::DeleteL3RouteIpv6(int unit, int vrf, const std::string& subnet,
  ...
  VLOG(1) << "Deleted IPv6 L3 LPM route " << PrintL3Route(route) << " on unit " << unit << ".";
...
::util::Status BcmSdkWrapper::DeleteL3HostIpv4(int unit, int vrf, uint32 ipv4) {
  ...
  VLOG(1) << "Deleted IPv4 L3 host route " << PrintL3Host(host) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::DeleteL3HostIpv6(int unit, int vrf, const std::string& ipv6) {
  uint64_t max;
  uint64_t min;
  uint64_t data;
  bool entry_delete = false;
  ...
  VLOG(1) << "Deleted IPv6 L3 host route " << PrintL3Host(host) << " on unit " << unit << ".";

  return ::util::OkStatus();
}

::util::StatusOr<int> BcmSdkWrapper::AddMyStationEntry(int unit, int priority, int vlan, int vlan_mask,
  ...
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L2_MY_STATIONs, VLAN_IDs, &min, &max));
  if ((vlan > static_cast<int>(max)) || (vlan < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vlan (" << vlan << "), valid vlan range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  // Check if vlan mask is valid
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L2_MY_STATIONs, VLAN_ID_MASKs, &min, &max));
  if ((vlan_mask > static_cast<int>(max)) || (vlan_mask < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid vlan_mask (" << vlan_mask
                                         << "), valid vlan_mask range is " << static_cast<int>(min) << " - "
                                         << static_cast<int>(max) << ".";
  }
  // Check if priority is valid
  RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, L2_MY_STATIONs, ENTRY_PRIORITYs, &min, &max));
  if ((priority > static_cast<int>(max)) || (priority < static_cast<int>(min))) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid priority (" << priority << "), valid priority range is "
                                         << static_cast<int>(min) << " - " << static_cast<int>(max) << ".";
  }
  ...
  CHECK_RETURN_IF_FALSE(unit_to_my_stations != nullptr) << "Unit " << unit
                                                        << "  is not found in unit_to_my_stations. Have you "
                                                        << "called InitializeUnit for this unit before?";
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, vlan));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_ID_MASKs, vlan_mask));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAC_ADDRs, dst_mac));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAC_ADDR_MASKs, dst_mac_mask));
  ...
  VLOG(1) << "Added dst MAC " << BcmMacToStr(mac) << "&&&" << BcmMacToStr(mac_mask) << " and VLAN " << vlan
          << " to my station TCAM with priority " << priority << " on unit " << unit << ".";
  return station_id;
}
...
::util::Status BcmSdkWrapper::DeleteMyStationEntry(int unit, int station_id) {
  ...
  if (!entry) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Station ID " << station_id << " not found.";
  }
  // delete entry
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L2_MY_STATIONs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, entry->vlan));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_ID_MASKs, entry->vlan_mask));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAC_ADDRs, entry->dst_mac));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAC_ADDR_MASKs, entry->dst_mac_mask));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  // delete map
...
::util::Status BcmSdkWrapper::AddL2Entry(int unit, int vlan, uint64 dst_mac, int logical_port, int trunk_port,
                                         int l2_mcast_group_id, int class_id, bool copy_to_cpu,
                                         bool dst_drop) {
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  bcmlt_entry_handle_t entry_hdl;
...
::util::Status BcmSdkWrapper::AddL2MulticastEntry(int unit, int priority, int vlan, int vlan_mask,
                                                  uint64 dst_mac, uint64 dst_mac_mask, bool copy_to_cpu,
                                                  bool drop, uint8 l2_mcast_group_id) {
  bcmlt_entry_handle_t entry_hdl;
  uint64_t max;
  uint64_t min;
...
::util::Status BcmSdkWrapper::DeleteL2EntriesByVlan(int unit, int vlan) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L2_FDB_VLANs, &entry_hdl));
  while (SHR_E_NONE == bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_TRAVERSE, BCMLT_PRIORITY_NORMAL)) {
    if (bcmlt_entry_info_get(entry_hdl, &entry_info) != SHR_E_NONE || entry_info.status != SHR_E_NONE) {
      break;
    }
    if (bcmlt_entry_field_get(entry_hdl, VLAN_IDs, &current_vlan) != SHR_E_NONE) {
      break;
    }
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, L2_FDB_VLAN_STATICs, &entry_hdl));
  while (SHR_E_NONE == bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_TRAVERSE, BCMLT_PRIORITY_NORMAL)) {
    if (bcmlt_entry_info_get(entry_hdl, &entry_info) != SHR_E_NONE || entry_info.status != SHR_E_NONE) {
      break;
    }
...
::util::Status BcmSdkWrapper::AddVlanIfNotFound(int unit, int vlan) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, EGR_MEMBER_PORTSs, 0, members, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, ING_MEMBER_PORTSs, 0, members, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, UNTAGGED_MEMBER_PORTSs, 0, untagged_members, 3));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, L3_IIF_IDs, 1));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
...
::util::Status BcmSdkWrapper::DeleteVlanIfFound(int unit, int vlan) {
  ...
  if (entry_info.status == SHR_E_NOT_FOUND) {
    ...
    RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, vlan));
    int retval = bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_DELETE, BCMLT_PRIORITY_NORMAL);
    if (retval != SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
      return MAKE_ERROR(ERR_INTERNAL) << "Failed to delete VLAN " << vlan << " from unit " << unit << ".";
    }
...
::util::Status BcmSdkWrapper::ConfigureVlanBlock(int unit, int vlan, bool block_broadcast,
                                                 bool block_known_multicast, bool block_unknown_multicast,
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, VLANs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_IDs, vlan));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
  if (entry_info.status == SHR_E_NOT_FOUND) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  if (entry_info.status == SHR_E_NOT_FOUND) {
    VLOG(1) << "VLAN profile " << static_cast<uint16>(profile_id) << " does not exist.";
    RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, VLAN_PROFILEs, &entry_hdl));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, VLAN_PROFILE_IDs, profile_id));
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  ...
  RETURN_IF_BCM_ERROR(
      bcmlt_entry_field_add(entry_hdl, L2_MISS_DROPs, block_unknown_multicast || block_unknown_unicast));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
...
::util::Status BcmSdkWrapper::ConfigureL2Learning(int unit, int vlan, bool disable_l2_learning) {
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  uint64 data;
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  uint8 profile_id = data;
  VLOG(1) << "VLAN " << vlan << " has VLAN profile " << static_cast<uint16>(profile_id);
  if (profile_id == 0) {
...
::util::Status BcmSdkWrapper::ConfigSerdesForPort(int unit, int port, uint64 speed_bps, int serdes_core,
...
::util::Status BcmSdkWrapper::CreateKnetIntf(int unit, int vlan, std::string* netif_name, int* netif_id) {
  CHECK_RETURN_IF_FALSE(netif_name != nullptr && netif_id != nullptr)
      << "Null netif_name or netif_id pointers.";
  CHECK_RETURN_IF_FALSE(!netif_name->empty()) << "Empty netif name for unit " << unit << ".";
  CHECK_RETURN_IF_FALSE(netif_name->length() <= static_cast<size_t>(BCMPKT_DEV_NAME_MAX))
  ...
  // netif.vlan = vlan > 0 ? vlan : kDefaultVlan;
  // TODO(unknown): Do we want VLAN tags on packetIO Tx packets?
  ...
  RETURN_IF_BCM_ERROR(bcmpkt_rx_register(unit, netif.id, 0, packet_receive_callback, NULL));
...
::util::StatusOr<int> BcmSdkWrapper::CreateKnetFilter(int unit, int netif_id, KnetFilterType type) {
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  ...
  switch (type) {
    ...
    case KnetFilterType::CATCH_SFLOW_FROM_INGRESS_PORT:
      ...
      snprintf(filter.desc, sizeof(filter.desc), "CATCH_SFLOW_FROM_INGRESS_PORT");
    ...
    case KnetFilterType::CATCH_SFLOW_FROM_EGRESS_PORT:
      ...
      snprintf(filter.desc, sizeof(filter.desc), "CATCH_SFLOW_FROM_EGRESS_PORT");
...
::util::Status BcmSdkWrapper::StartRx(int unit, const RxConfig& rx_config) {
  ...
  char kCliChannelMapString[] = "pktdev chan queuemap 1 highword=0xffff lowword=0xffffffff";
...
::util::Status BcmSdkWrapper::StopRx(int unit) { return ::util::OkStatus(); }

::util::Status BcmSdkWrapper::SetRateLimit(int unit, const RateLimitConfig& rate_limit_config) {
  uint64_t max;
  uint64_t min;
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  RETURN_IF_BCM_ERROR(
      GetFieldMinMaxValue(unit, TM_SCHEDULER_SHAPER_CPU_NODEs, TM_SCHEDULER_SHAPER_CPU_NODE_IDs, &min, &max));
  // Sanity checking.
  ...
  // hardcoding CPU PORT
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, PORT_IDs, 0));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, BANDWIDTH_KBPSs, rate_limit_config.max_rate_pps));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, BURST_SIZE_KBITSs, rate_limit_config.max_burst_pkts));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, SHAPING_MODEs, PACKET_MODEs));
  if (entry_info.status == SHR_E_NONE) {
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
  } else {
    RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, TM_SCHEDULER_SHAPER_CPU_NODEs, &entry_hdl));
  for (const auto& e : rate_limit_config.per_cos_rate_limit_configs) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, SCHED_NODEs, L1_SCHED_NODEs));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, TM_SCHEDULER_SHAPER_CPU_NODE_IDs, e.first));
    RETURN_IF_BCM_ERROR(bcmlt_entry_commit(entry_hdl, BCMLT_OPCODE_LOOKUP, BCMLT_PRIORITY_NORMAL));
    RETURN_IF_BCM_ERROR(bcmlt_entry_info_get(entry_hdl, &entry_info));
    RETURN_IF_BCM_ERROR(bcmlt_entry_clear(entry_hdl));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, SCHED_NODEs, L1_SCHED_NODEs));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, TM_SCHEDULER_SHAPER_CPU_NODE_IDs, e.first));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAX_BURST_SIZE_KBITSs, e.second.max_burst_pkts));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAX_BANDWIDTH_KBPSs, e.second.max_rate_pps));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, SHAPING_MODEs, PACKET_MODEs));
    if (entry_info.status == SHR_E_NONE) {
      RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_UPDATE, BCMLT_PRIORITY_NORMAL));
    } else {
...
::util::Status BcmSdkWrapper::GetKnetHeaderForDirectTx(int unit, int port, int cos, uint64 smac,
  ...
  std::string s0(reinterpret_cast<const char*>(&rcpu_header), sizeof(rcpu_header));
  ...
  CHECK_RETURN_IF_FALSE(chip_type == BcmChip::TOMAHAWK) << "Un-supported BCM chip type: "
...
#define TXPMD_HEADER_TYPE_FROM_CPU 1
  ...
  RETURN_IF_BCM_ERROR(
      bcmpkt_txpmd_field_set(dev_type, meta, BCMPKT_TXPMD_HEADER_TYPE, TXPMD_HEADER_TYPE_FROM_CPU));
  ...
  VLOG(2) << "txpmd: " << StringToHex(std::string(reinterpret_cast<const char*>(meta), sizeof(meta)));
  header->append(reinterpret_cast<const char*>(meta), sizeof(meta));

  return ::util::OkStatus();
}

::util::Status BcmSdkWrapper::GetKnetHeaderForIngressPipelineTx(int unit, uint64 smac, size_t packet_len,
                                                                std::string* header) {
  CHECK_RETURN_IF_FALSE(header != nullptr);
  header->clear();
...
size_t BcmSdkWrapper::GetKnetHeaderSizeForRx(int unit) { return sizeof(RcpuHeader) + kRcpuRxMetaSize; }
...
::util::Status BcmSdkWrapper::ParseKnetHeaderForRx(int unit, const std::string& header,
                                                   int* ingress_logical_port, int* egress_logical_port,
                                                   int* cos) {
  ...
  const struct RcpuHeader* rcpu_header = reinterpret_cast<const struct RcpuHeader*>(header.data());
  CHECK_RETURN_IF_FALSE(ntohs(rcpu_header->ether_header.ether_type) == kRcpuVlanEthertype)
      << ntohs(rcpu_header->ether_header.ether_type) << " != " << kRcpuVlanEthertype;
  CHECK_RETURN_IF_FALSE((ntohs(rcpu_header->vlan_tag.vlan_id) & kVlanIdMask) == kRcpuVlanId)
      << (ntohs(rcpu_header->vlan_tag.vlan_id) & kVlanIdMask) << " != " << kRcpuVlanId;
  CHECK_RETURN_IF_FALSE(ntohs(rcpu_header->vlan_tag.type) == kRcpuEthertype)
      << ntohs(rcpu_header->vlan_tag.type) << " != " << kRcpuEthertype;
  CHECK_RETURN_IF_FALSE(rcpu_header->rcpu_data.rcpu_opcode == kRcpuOpcodeToCpuPkt)
  ...
  int src_module = -1, dst_module = -1, src_port = -1, dst_port = -1, op_code = -1;
  ...
  *cos = val;
  ...
  int module = 0;
  VLOG(1) << "Parsed metadata: (op_code=" << op_code << ", src_mod=" << src_module
          << ", dst_mod=" << dst_module << ", base_mod=" << module << ", src_port=" << src_port
  ...
  CHECK_RETURN_IF_FALSE(src_module == module) << "Invalid src_module: (op_code=" << op_code
  ...
  switch (op_code) {
    ...
    case 1:  // BCMPKT_OPCODE_UC
      CHECK_RETURN_IF_FALSE(dst_module == module) << "Invalid dst_module: (op_code=" << op_code
                                                  << ", src_mod=" << src_module << ", dst_mod=" << dst_module
                                                  << ", base_mod=" << module << ", src_port=" << src_port
                                                  << ", dst_port=" << dst_port << ", cos=" << *cos << ").";
    ...
    case 2:  // BCMPKT_OPCODE_BC
      ...
      *egress_logical_port = 0;  // CPU port
      break;
    default:
      return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid op_code: (op_code=" << op_code
                                           << ", src_mod=" << src_module << ", dst_mod=" << dst_module
                                           << ", base_mod=" << module << ", src_port=" << src_port
                                           << ", dst_port=" << dst_port << ", cos=" << *cos << ").";
  }
...
::util::Status BcmSdkWrapper::SetAclControl(int unit, const AclControl& acl_control) {
  bcmlt_entry_handle_t entry_hdl;
  // Check if the unit is valid
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
...
std::vector<std::pair<std::string, std::string>> GetPktTypeAndMode(std::vector<std::string> qualifiers) {
  // L2_SINGLE_WIDE, PORT_ANY_PACKET_ANY
  std::vector<std::string> PortAnyPktAnyL2SingleWide = {
      QUAL_L4_PKTs,
      QUAL_EGR_NHOP_CLASS_IDs,
      QUAL_EGR_L3_INTF_CLASS_IDs,
      QUAL_EGR_DVP_CLASS_IDs,
      QUAL_DST_VPs,
      QUAL_DST_VP_VALIDs,
      QUAL_INT_PRIs,
      QUAL_COLORs,
      ...
      QUAL_SRC_MACs,
      QUAL_DST_MACs,
      QUAL_VXLT_LOOKUP_HITs,
      QUAL_INNER_VLAN_CFIs,
      QUAL_INNER_VLAN_PRIs,
      QUAL_INNER_VLAN_IDs,
      QUAL_INPORTs,
      QUAL_L3_ROUTABLE_PKTs,
      QUAL_MIRR_COPYs,
      QUAL_OUTER_VLAN_IDs,
      QUAL_OUTER_VLAN_CFIs,
      QUAL_OUTER_VLAN_PRIs,
      QUAL_VLAN_INNER_PRESENTs,
      QUAL_VLAN_OUTER_PRESENTs,
      QUAL_OUTPORTs,
      QUAL_CPU_COSs,
      QUAL_IP_TYPEs,
      QUAL_FWD_VLAN_IDs,
  ...
  std::vector<std::string> PortAnyPktIPV4L3SingleWide = {
      ...
      QUAL_COLORs,
      QUAL_IP_FLAGS_MFs,
      QUAL_TCP_CONTROL_FLAGSs,
      QUAL_L4DST_PORTs,
      QUAL_L4SRC_PORTs,
      QUAL_ICMP_TYPE_CODEs,
      QUAL_TTLs,
      QUAL_IP_PROTOCOLs,
      QUAL_DST_IP4s,
      QUAL_SRC_IP4s,
  ...
  std::vector<std::string> PortAnyPktIPV6L3DoubleWide = {
      ...
      QUAL_ICMP_TYPE_CODEs,
      QUAL_TTLs,
      QUAL_IP_FIRST_EH_SUBCODEs,
      QUAL_IP_FIRST_EH_PROTOs,
      QUAL_DROP_PKTs,
  };
  ...
  std::vector<std::string> PortLbkPktAnyL3AnyDoubleWide = {
      QUAL_LOOPBACK_QUEUEs,
      QUAL_LOOPBACK_TYPEs,
      QUAL_PKT_IS_VISIBLEs,
      QUAL_LOOPBACK_CPU_MSQRD_PKT_PROFs,
      QUAL_LOOPBACK_COLORs,
      QUAL_LOOPBACK_TRAFFIC_CLASSs,
      QUAL_LOOPBACK_PKT_PROCESSING_PORTs,
      QUAL_INPORTs,
      QUAL_OUTPORTs,
      QUAL_INT_PRIs,
      QUAL_COLORs,
  ...
  if (std::includes(PortAnyPktAnyL2SingleWide.begin(), PortAnyPktAnyL2SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(std::make_pair(L2_SINGLE_WIDEs, PORT_ANY_PACKET_ANYs));
  }

  if (std::includes(PortAnyPktIPV4L3SingleWide.begin(), PortAnyPktIPV4L3SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(std::make_pair(L3_SINGLE_WIDEs, PORT_ANY_PACKET_IPV4s));
  }

  if (std::includes(PortAnyPktIPV6L3SingleWide.begin(), PortAnyPktIPV6L3SingleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
  ...
  if (std::includes(PortAnyPktIPV4L3DoubleWide.begin(), PortAnyPktIPV4L3DoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
    possible_combination.push_back(std::make_pair(L3_DOUBLE_WIDEs, PORT_ANY_PACKET_IPV4s));
  }

  if (std::includes(PortAnyPktIPV6L3DoubleWide.begin(), PortAnyPktIPV6L3DoubleWide.end(), qualifiers.begin(),
                    qualifiers.end())) {
...
std::pair<std::string, std::string> HalAclFieldToBcmRule(BcmAclStage stage, BcmField::Type field) {
  // EFP specific field mappings.
  static auto* efp_field_map =
      new absl::flat_hash_map<BcmField::Type, std::pair<std::string, std::string>, EnumHash<BcmField::Type>>({
...
std::string HalAclFieldToBcm(BcmAclStage stage, BcmField::Type field) {
  // EFP specific field mappings.
  static auto* efp_field_map =
      new absl::flat_hash_map<BcmField::Type, std::string, EnumHash<BcmField::Type>>({
...
::util::StatusOr<int> GetUniqueId(std::map<std::pair<BcmAclStage, int>, int>* table_ids, int id, int max) {
  int acl_id = 0;
...
::util::Status CreateVfpGroup(int unit, int stage_id, const BcmAclTable& table) {
  bcmlt_entry_handle_t entry_hdl;
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, ENTRY_PRIORITYs, table.priority()));
  for (const auto& field : table.fields()) {
    if (field.udf_chunk_id()) {
      ...
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, QUAL_UDF_CHUNKSs, index, &value, 1));
      continue;
    }
    std::string bcm_qual_field = HalAclFieldToBcm(table.stage(), field.type());
    std::string unknown_qual = BcmField_Type_Name(BcmField::UNKNOWN);
    if ((unknown_qual.compare(bcm_qual_field)) == 0) {
      return MAKE_ERROR(ERR_INVALID_PARAM) << "Attempted to create ACL table with invalid predefined "
                                           << " qualifier: " << field.ShortDebugString() << ".";
    }
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, bcm_qual_field.c_str(), 1));
  }
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status CreateIfpGroup(int unit, int stage_id, const BcmAclTable& table) {
  uint64_t max;
  uint64_t min;
  bcmlt_entry_handle_t entry_hdl;
  ...
  for (const auto& field : table.fields()) {
    if (field.udf_chunk_id()) {
      uint32_t index = static_cast<uint32>(field.udf_chunk_id() - 1);
      uint64_t value = static_cast<uint64>(0xffff);
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, QUAL_UDF_CHUNKS_BITMAPs, index, &value, 1));
      continue;
    }
    std::string bcm_qual_field = HalAclFieldToBcm(table.stage(), field.type());
    std::string unknown_qual = BcmField_Type_Name(BcmField::UNKNOWN);
    ...
    RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_ING_GRP_TEMPLATEs, bcm_qual_field.c_str(), &min, &max));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, bcm_qual_field.c_str(), max));
  }
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  return ::util::OkStatus();
}

::util::Status CreateEfpGroup(int unit, int stage_id, const BcmAclTable& table) {
  ...
  for (const auto& field : table.fields()) {
    if (field.udf_chunk_id()) {
      return MAKE_ERROR(ERR_INTERNAL) << "UDF is not valid in " << BcmAclStage_Name(BCM_ACL_STAGE_EFP) << ".";
    }
    std::string bcm_qual_field = HalAclFieldToBcm(table.stage(), field.type());
    std::string unknown_qual = BcmField_Type_Name(BcmField::UNKNOWN);
    if ((unknown_qual.compare(bcm_qual_field)) == 0) {
  ...
  std::vector<std::pair<std::string, std::string>> possibles = GetPktTypeAndMode(efp_qualifiers);
  if (possibles.size() > 0) {
    for (const auto& p : possibles) {
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, PORT_PKT_TYPEs, p.second.c_str()));
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, MODEs, p.first.c_str()));
      int rv = bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL);
      if (rv == SHR_E_NONE) {
        break;
      }
    }
  } else {
    return MAKE_ERROR(ERR_INTERNAL) << "Failed to create ACL Table in " << BcmAclStage_Name(BCM_ACL_STAGE_EFP)
                                    << ".";
  }
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status CreateAclGroup(int unit, int id, BcmAclStage stage, const BcmAclTable& table) {
  if (stage == BCM_ACL_STAGE_VFP) {
    ...
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "Attempted to create ACL table with invalid pipeline stage: " << BcmAclStage_Name(stage) << ".";
  }
  return ::util::OkStatus();
}
...
::util::StatusOr<int> BcmSdkWrapper::CreateAclTable(int unit, const BcmAclTable& table) {
  int stage_id;
  int table_id;
  InUseMap* group_ids;
...
::util::Status BcmSdkWrapper::DestroyAclTable(int unit, int table_id) {
  ...
  CHECK_RETURN_IF_FALSE(table_ids != nullptr) << "Unit " << unit
                                              << " not initialized yet. Call InitializeUnit first.";

  ASSIGN_OR_RETURN(found, FindAndReturnEntry(table_ids, table_id, &entry));
  if (!found) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "ACL Table id " << table_id << " not found.";
  }
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    ...
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "ACL table with invalid pipeline stage: " << BcmAclStage_Name(stage) << ".";
  }
  CHECK_RETURN_IF_FALSE(group_ids != nullptr) << "Unit " << unit
                                              << " not initialized yet. Call InitializeUnit first.";
  ...
  if (entry_deleted) {
    ...
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Failed to delete ACL table with id: " << table_id << " in stage "
...
::util::Status AddAclQualifier(int unit, const bcmlt_entry_handle_t& entry_hdl,
                               const std::pair<std::string, std::string> field_pair, const BcmAclStage& stage,
  ...
  if (field_mask_name.length() > 0) {
    if (stage == BCM_ACL_STAGE_VFP) {
      RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_VLAN_RULE_TEMPLATEs, field_mask_name.c_str(),
                                              &min_mask64, &max_mask64));
    } else if (stage == BCM_ACL_STAGE_IFP) {
      RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_ING_RULE_TEMPLATEs, field_mask_name.c_str(),
                                              &min_mask64, &max_mask64));
    } else if (stage == BCM_ACL_STAGE_EFP) {
      RETURN_IF_BCM_ERROR(GetFieldMinMaxValue(unit, FP_EGR_RULE_TEMPLATEs, field_mask_name.c_str(),
                                              &min_mask64, &max_mask64));
    } else {
      return MAKE_ERROR(ERR_INVALID_PARAM)
             << "Attempted to create ACL rule with invalid pipeline stage: " << BcmAclStage_Name(stage)
             << ".";
    }
  }
  ...
  switch (field.type()) {
    ...
    case BcmField::IP_TYPE: {
      if (field.has_mask()) {
        return MAKE_ERROR(ERR_INVALID_PARAM) << "IpType metadata qualifier contained unexpected mask entry.";
      }
      std::string ip_type = "";
      switch (field.value().u32()) {
        ...
        case kEtherTypeArp:
          ...
          break;
        default:
          return MAKE_ERROR(ERR_INVALID_PARAM) << "IpType metadata qualifier contained unsupported EtherType "
                                                  "value.";
      }
...
::util::Status CreateVfpRule(int unit, int rule_id, const BcmFlowEntry& flow) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_VLAN_RULE_TEMPLATE_IDs, rule_id));
  for (const auto& field : flow.fields()) {
    if (field.udf_chunk_id()) {
      ...
      if (field.value().b().size() != 2 ||
        ...
        return MAKE_ERROR(ERR_INVALID_PARAM) << "Attempted to program flow with UDF chunk "
                                             << field.udf_chunk_id()
                                             << " with value or mask size not equal to "
                                             << "chunk size " << 2 << ".";
      }
      uint32_t index = static_cast<uint32>(field.udf_chunk_id() - 1);
      ...
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, QUAL_UDF_CHUNKSs, index, &value64, 1));
      RETURN_IF_BCM_ERROR(bcmlt_entry_field_array_add(entry_hdl, QUAL_UDF_CHUNKS_MASKs, index, &mask64, 1));
      continue;
    }
    ...
    if (bcm_qual_field.first == BcmField_Type_Name(BcmField::UNKNOWN)) {
      LOG(INFO) << "Qual: '" << field.ShortDebugString() << "' in " << BcmAclStage_Name(BCM_ACL_STAGE_VFP)
                << ".";
      return MAKE_ERROR() << "Attempted to translate unsupported BcmField::Type: "
                          << BcmField::Type_Name(field.type()) << ".";
    }
    RETURN_IF_ERROR(AddAclQualifier(unit, entry_hdl, bcm_qual_field, flow.acl_stage(), field));
  }
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
...
::util::Status CreateIfpRule(int unit, int rule_id, const BcmFlowEntry& flow) {
  ...
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_RULE_TEMPLATE_IDs, rule_id));
  for (const auto& field : flow.fields()) {
    if (field.udf_chunk_id()) {
      // TODO(BRCM): hardcoding kUdfChunkSize: 2,
      // this should be ok for Tomahawk, revisit if this is a problem
    ...
    std::pair<std::string, std::string> bcm_qual_field = HalAclFieldToBcmRule(flow.acl_stage(), field.type());
    if (bcm_qual_field.first == BcmField_Type_Name(BcmField::UNKNOWN)) {
      LOG(INFO) << "Qual: '" << field.ShortDebugString() << "' in " << BcmAclStage_Name(BCM_ACL_STAGE_IFP)
                << ".";
      return MAKE_ERROR() << "Attempted to translate unsupported BcmField::Type: "
                          << BcmField::Type_Name(field.type()) << ".";
    }
...
::util::Status VerifyAclActionParams(
    ...
    const absl::flat_hash_set<BcmAction::Param::Type, EnumHash<BcmAction::Param::Type>>& required,
    const absl::flat_hash_set<BcmAction::Param::Type, EnumHash<BcmAction::Param::Type>>& optional) {
  ...
  for (const auto& param : action.params()) {
    if (!(req_params.erase(param.type()) || opt_params.erase(param.type()))) {
      return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid or duplicate parameter for "
  ...
  if (!req_params.empty()) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Unmatched parameter(s) for action: " << action.ShortDebugString()
                                         << ".";
  }
  return ::util::OkStatus();
}

::util::Status AddAclAction(int unit, const bcmlt_entry_handle_t& entry_hdl, int policy_id,
                            const BcmAclStage& stage, const BcmAction& action) {
  // Sets of required and optional action parameters.
  absl::flat_hash_set<BcmAction::Param::Type, EnumHash<BcmAction::Param::Type>> required;
  absl::flat_hash_set<BcmAction::Param::Type, EnumHash<BcmAction::Param::Type>> optional;
  switch (action.type()) {
    case BcmAction::DROP: {
      ...
      switch (action.params(0).value().u32()) {
        case BcmAction::Param::GREEN:
          if (stage == BCM_ACL_STAGE_IFP || stage == BCM_ACL_STAGE_EFP) {
            ...
            return MAKE_ERROR(ERR_INVALID_PARAM) << "Attempted to translate unsupported BcmAction::Type: "
        ...
        default:
          return MAKE_ERROR(ERR_INVALID_PARAM)
                 << "Invalid color parameter for DROP action: " << action.params(0).value().u32() << ".";
      }
    } break;
        ...
        switch (param.type()) {
          ...
          case BcmAction::Param::COLOR:
            switch (param.value().u32()) {
              case BcmAction::Param::GREEN:
                ...
                if (stage == BCM_ACL_STAGE_IFP) {
                  ...
                  return MAKE_ERROR(ERR_INVALID_PARAM)
                         ...
                         << BcmAction::Type_Name(action.type()) << " in " << BcmAclStage_Name(stage) << ".";
                }
                break;
              case BcmAction::Param::YELLOW:
              ...
              default:
                return MAKE_ERROR(ERR_INVALID_PARAM)
                       << "Invalid color parameter for COPY_TO_CPU action: " << param.value().u32() << ".";
            }
            break;
          default:
            return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid parameter type for COPY_TO_CPU action: "
                                                 << BcmAction::Param::Type_Name(param.type()) << ".";
        }
      }
      if (!colorFound) {
    ...
    case BcmAction::CANCEL_COPY_TO_CPU: {
      ...
      switch (action.params(0).value().u32()) {
        ...
        case BcmAction::Param::RED:
          ...
          break;
        default:
          return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid color parameter for CANCEL_COPY_TO_CPU "
      ...
      switch (color) {
        case BcmAction::Param::GREEN:
          if (stage == BCM_ACL_STAGE_VFP) {
            ...
            return MAKE_ERROR(ERR_INVALID_PARAM) << "Invalid color parameter for SET_COLOR action: " << color
                                                 << ".";
          }
          break;
        case BcmAction::Param::YELLOW:
    ...
    default:
      return MAKE_ERROR(ERR_INVALID_PARAM)
             << "Attempted to translate unsupported BcmAction::Type: " << BcmAction::Type_Name(action.type())
             << ".";
  }
  return ::util::OkStatus();
}

::util::Status CreateVfpPolicy(int unit, int policy_id, const BcmFlowEntry& flow) {
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_VLAN_POLICY_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_VLAN_POLICY_TEMPLATE_IDs, policy_id));
  // Translate actions and add to new flow entry.
  for (const auto& action : flow.actions()) {
    RETURN_IF_ERROR(AddAclAction(unit, entry_hdl, policy_id, flow.acl_stage(), action));
  }
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
  RETURN_IF_BCM_ERROR(bcmlt_entry_free(entry_hdl));
  return ::util::OkStatus();
}
::util::Status CreateIfpPolicy(int unit, int policy_id, const BcmFlowEntry& flow) {
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_ING_POLICY_TEMPLATEs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_POLICY_TEMPLATE_IDs, policy_id));
  // Translate actions and add to new flow entry.
  for (const auto& action : flow.actions()) {
    RETURN_IF_ERROR(AddAclAction(unit, entry_hdl, policy_id, flow.acl_stage(), action));
  }
...
::util::Status CreateEfpPolicy(int unit, int policy_id, const BcmFlowEntry& flow) {
  bcmlt_entry_handle_t entry_hdl;
...
// parameters.
::util::Status AddAclPolicer(int unit, int meter_id, int stage, const BcmMeterConfig& meter) {
  bcmlt_entry_handle_t entry_hdl;
  if (stage == BCM_ACL_STAGE_IFP) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, METER_FP_ING_TEMPLATEs, &entry_hdl));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, METER_FP_ING_TEMPLATE_IDs, meter_id));
  } else if (stage == BCM_ACL_STAGE_EFP) {
    ...
    return MAKE_ERROR(ERR_INVALID_PARAM) << "TODO(BRCM): Add proper error message";
  }
  // Determine whether the meter is to be configured for a single rate (two
  // colors) or for trTCM mode.
  if ((meter.committed_rate() == meter.peak_rate()) && (meter.committed_burst() == meter.peak_burst())) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, METER_MODEs, FLOWs));
  } else {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_symbol_add(entry_hdl, METER_MODEs, TRTCMs));
    uint32 peak_rate = meter.peak_rate();
    uint32 peak_burst = meter.peak_burst();
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAX_RATE_KBPSs, peak_rate));
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MAX_BURST_SIZE_KBITSs, peak_burst));
  }
  uint32 committed_rate = meter.committed_rate();
  uint32 committed_burst = meter.committed_burst();
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MIN_RATE_KBPSs, committed_rate));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, MIN_BURST_SIZE_KBITSs, committed_burst));
...
::util::Status CreateVfpEntry(int unit, int acl_id, int priority, int group_id, int rule_id, int policy_id) {
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_VLAN_ENTRYs, &entry_hdl));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_VLAN_ENTRY_IDs, acl_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_VLAN_GRP_TEMPLATE_IDs, group_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_VLAN_RULE_TEMPLATE_IDs, rule_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_VLAN_POLICY_TEMPLATE_IDs, policy_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, ENTRY_PRIORITYs, priority));
  RETURN_IF_BCM_ERROR(bcmlt_custom_entry_commit(entry_hdl, BCMLT_OPCODE_INSERT, BCMLT_PRIORITY_NORMAL));
...
::util::Status CreateIfpEntry(int unit, int acl_id, int priority, int group_id, int rule_id, int policy_id,
                              int meter_id) {
  bcmlt_entry_handle_t entry_hdl;
  RETURN_IF_BCM_ERROR(bcmlt_entry_allocate(unit, FP_ING_ENTRYs, &entry_hdl));
  auto _ = gtl::MakeCleanup([entry_hdl]() { bcmlt_entry_free(entry_hdl); });
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_ENTRY_IDs, acl_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_GRP_TEMPLATE_IDs, group_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_RULE_TEMPLATE_IDs, rule_id));
  RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, FP_ING_POLICY_TEMPLATE_IDs, policy_id));
  if (meter_id > 0) {
    RETURN_IF_BCM_ERROR(bcmlt_entry_field_add(entry_hdl, METER_FP_ING_TEMPLATE_IDs, meter_id));
...
::util::Status CreateEfpEntry(int unit, int acl_id, int priority, int group_id, int rule_id, int policy_id,
                              int meter_id) {
  bcmlt_entry_handle_t entry_hdl;
...
::util::StatusOr<int> BcmSdkWrapper::InsertAclFlow(int unit, const BcmFlowEntry& flow, bool add_stats,
                                                   bool color_aware) {
  int rule_id = 0;
  int policy_id = 0;
  int meter_id = 0;
  ...
  if (!found) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "ACL Table id " << flow.bcm_acl_table_id() << " not found.";
  }
  BcmAclStage stage = flow.acl_stage();
  CHECK_RETURN_IF_FALSE(stage == entry.first) << "Invalid valid group, stage used for group is "
                                              << BcmAclStage_Name(entry.first)
                                              << " stage used for the flow is " << BcmAclStage_Name(stage);
  ...
  switch (stage) {
    ...
    default:
      return MAKE_ERROR(ERR_INVALID_PARAM)
             << "Attempted to create ACL flow with invalid pipeline stage: " << BcmAclStage_Name(stage)
             << ".";
  }

  if (!rule_ids || !policy_ids || !acl_ids) {
    return MAKE_ERROR(ERR_INTERNAL) << "TODO(BRCM): Add proper error message";
  }
  ...
  ASSIGN_OR_RETURN(policy_table_id, GetUniqueId(policy_table_ids, -1, maxPolicies));
  ...
  if (flow.has_meter()) {
    if (stage == BCM_ACL_STAGE_IFP || stage == BCM_ACL_STAGE_EFP) {
      ...
      ASSIGN_OR_RETURN(meter_table_id, GetUniqueId(meter_table_ids, -1, maxMeters));
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    RETURN_IF_ERROR(CreateVfpEntry(unit, acl_id, flow.priority(), group_id, rule_id, policy_id));
  } else if (stage == BCM_ACL_STAGE_IFP) {
    RETURN_IF_ERROR(CreateIfpEntry(unit, acl_id, flow.priority(), group_id, rule_id, policy_id, meter_id));
  } else {
    RETURN_IF_ERROR(CreateEfpEntry(unit, acl_id, flow.priority(), group_id, rule_id, policy_id, meter_id));
  }
...
::util::Status BcmSdkWrapper::ModifyAclFlow(int unit, int flow_id, const BcmFlowEntry& flow) {
  bool found;
  std::pair<BcmAclStage, int> entry;
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  ...
  if (!found) {
    return MAKE_ERROR(ERR_INVALID_PARAM) << "Flow id " << flow_id << " not found.";
  }

  BcmAclStage stage = flow.acl_stage();
  CHECK_RETURN_IF_FALSE(stage == entry.first) << "Stage used for the folw is not matching.";
  ...
  if (stage == BCM_ACL_STAGE_VFP) {
    ...
    RETURN_IF_ERROR(
        CreateIfpEntry(unit, acl_entry_id, flow.priority(), group_id, rule_id, policy_id, meter_id));
  } else if (stage == BCM_ACL_STAGE_EFP) {
    // TODO(BRCM): need to check if the flow is valid,
    // otherwise we should not delete existing flow
    ...
    RETURN_IF_ERROR(
        CreateEfpEntry(unit, acl_entry_id, flow.priority(), group_id, rule_id, policy_id, meter_id));
  } else {
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "Flow table with invalid pipeline stage: " << BcmAclStage_Name(stage) << ".";
  }
  return ::util::OkStatus();
}
...
::util::Status BcmSdkWrapper::SetAclPolicer(int unit, int flow_id, const BcmMeterConfig& meter) {
  bool found;
  std::pair<BcmAclStage, int> entry;
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  ...
  CHECK_RETURN_IF_FALSE((stage == BCM_ACL_STAGE_IFP) || (stage == BCM_ACL_STAGE_EFP))
      << "Meters can not be created/modified in the stage " << BcmAclStage_Name(stage) << ".";
...
::util::Status BcmSdkWrapper::InsertPacketReplicationEntry(const BcmPacketReplicationEntry& entry) {
  ...
  CHECK_RETURN_IF_FALSE(!gtl::ContainsKey(multicast_group_id_to_replicas_, mcast_entry.multicast_group_id()))
...
::util::Status BcmSdkWrapper::DeletePacketReplicationEntry(const BcmPacketReplicationEntry& entry) {
  // Check pre-conditions
  CHECK_RETURN_IF_FALSE(entry.has_multicast_group_entry());
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(entry.unit()));
  auto mcast_entry = entry.multicast_group_entry();
  auto ports = gtl::FindOrNull(multicast_group_id_to_replicas_, mcast_entry.multicast_group_id());
...
namespace {
::util::Status GetGroupDetails(int unit, int stage_id, int table_id, BcmAclStage stage, BcmAclTable* table) {
  uint64_t value = 0;
  uint32_t configured;
  bcmlt_entry_handle_t entry_hdl;
  bcmlt_entry_info_t entry_info;
  ...
  if (entry_info.status == SHR_E_NONE) {
    ...
    if (stage == BCM_ACL_STAGE_VFP) {
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_array_get(entry_hdl, QUAL_UDF_CHUNKSs, 0, chunk_array, 16, &num_chunks));
    } else if (stage == BCM_ACL_STAGE_IFP) {
      RETURN_IF_BCM_ERROR(
          bcmlt_entry_field_array_get(entry_hdl, QUAL_UDF_CHUNKS_BITMAPs, 0, chunk_array, 16, &num_chunks));
    }
    if (num_chunks) {
...
::util::Status BcmSdkWrapper::GetAclTable(int unit, int table_id, BcmAclTable* table) {
  bool found;
  std::pair<BcmAclStage, int> entry;
  ...
  BcmAclStage stage = entry.first;
  int stage_id = entry.second;
  if ((stage == BCM_ACL_STAGE_VFP) || (stage == BCM_ACL_STAGE_IFP) || (stage == BCM_ACL_STAGE_EFP)) {
    RETURN_IF_ERROR(GetGroupDetails(unit, stage_id, table_id, stage, table));
  } else {
    return MAKE_ERROR(ERR_INTERNAL) << "ACL table with invalid pipeline stage: " << BcmAclStage_Name(stage)
                                    << ".";
  }
  return ::util::OkStatus();
}
...
inline int bcm_get_field_u32(F func, int unit, int flow_id, uint32* value, uint32* mask) {
  T t_value, t_mask;
...
::util::Status BcmSdkWrapper::GetAclFlow(int unit, int flow_id, BcmFlowEntry* flow) {
  // TODO(unknown): Implement this function.
  return ::util::OkStatus();
}

::util::StatusOr<std::string> BcmSdkWrapper::MatchAclFlow(int unit, int flow_id, const BcmFlowEntry& flow) {
  // TODO(unknown): Implement this function.
  return std::string();
}

::util::Status BcmSdkWrapper::GetAclTableFlowIds(int unit, int table_id, std::vector<int>* flow_ids) {
  RETURN_IF_BCM_ERROR(CheckIfUnitExists(unit));
  auto* table_ids = gtl::FindPtrOrNull(fp_group_ids_, unit);
  CHECK_RETURN_IF_FALSE(table_ids != nullptr) << "Unit " << unit
                                              << " not initialized yet. Call InitializeUnit first.";
...
::util::Status BcmSdkWrapper::AddAclStats(int unit, int table_id, int flow_id, bool color_aware) {
  return MAKE_ERROR(ERR_FEATURE_UNAVAILABLE) << "Not supported.";
}
...
::util::Status BcmSdkWrapper::GetAclStats(int unit, int flow_id, BcmAclStats* stats) {
...
::util::Status BcmSdkWrapper::CleanupKnet(int unit) {
  ...
  RETURN_IF_BCM_ERROR(bcmpkt_filter_traverse(unit, knet_filter_remover, nullptr));
  RETURN_IF_BCM_ERROR(bcmpkt_netif_traverse(unit, knet_intf_remover, nullptr));
  return ::util::OkStatus();
}
...
::util::StatusOr<std::string> BcmSdkWrapper::FindSdkCheckpointFilePath(int unit) {
  return MAKE_ERROR(ERR_FEATURE_UNAVAILABLE) << "Not supported.";
}
...
::util::StatusOr<BcmChip::BcmChipType> BcmSdkWrapper::GetChipType(int unit) {
  ...
  if (it == unit_to_chip_type_.end()) {
    return MAKE_ERROR(ERR_INTERNAL) << "Unit " << unit << "  is not found in unit_to_chip_type_. Have you "
                                    << "called FindUnit for this unit before?";
  }
  return it->second;
}

::util::Status BcmSdkWrapper::SetIntfAndConfigurePhyForPort(int unit, int port,
                                                            BcmChip::BcmChipType chip_type, uint64 speed_bps,
                                                            const std::string& intf_type) {
  return MAKE_ERROR(ERR_FEATURE_UNAVAILABLE) << "Not supported.";
}

::util::Status BcmSdkWrapper::SetSerdesRegisterForPort(int unit, int port, BcmChip::BcmChipType chip_type,
                                                       int serdes_lane, uint32 reg, uint32 value) {
  return MAKE_ERROR(ERR_FEATURE_UNAVAILABLE) << "Not supported.";
}

::util::Status BcmSdkWrapper::SetSerdesAttributeForPort(int unit, int port, BcmChip::BcmChipType chip_type,
                                                        const std::string& attr, uint32 value) {
  return MAKE_ERROR(ERR_FEATURE_UNAVAILABLE) << "Not supported.";
}
