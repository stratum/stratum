...
class LibcProxyMock : public PassthroughLibcProxy {
 public:
  ...
  int bind(int sockfd, struct sockaddr* my_addr, socklen_t addrlen) override {
  ...
  MOCK_METHOD3(Bind,
               int(int sockfd, struct sockaddr* my_addr, socklen_t addrlen));
 ...
 protected:
  void SetUp() override {
    ...
    bcm_packetio_manager_ = BcmPacketioManager::CreateInstance(
        mode_, bcm_chassis_manager_mock_.get(), p4_table_mapper_mock_.get(),
  ...
  ::util::Status PopulateChassisConfigAndPortMaps(
    ...
    if (port_id_to_unit_logical_port) {
      // One port on unit1 and one port on unit2.
      port_id_to_unit_logical_port->insert({kPortId1, {kUnit1, kLogicalPort1}});
      port_id_to_unit_logical_port->insert({kPortId2, {kUnit2, kLogicalPort2}});
    }
  ...
  void VerifyInternalStateAfterConfigPush(uint64 node_id) const {
    ...
    if (node_id == kNodeId1) {
      ...
      EXPECT_TRUE(bcm_packetio_manager_->trunk_id_to_member_port_ids_.empty());
      EXPECT_TRUE(bcm_packetio_manager_->port_id_to_parent_trunk_id_.empty());
  ...
  static constexpr uint64 kPortId1 = 1111;
  static constexpr uint64 kPortId2 = 2222;
  static constexpr uint64 kPortId3 = 3333;
...
constexpr uint64 BcmPacketioManagerTest::kPortId1;
constexpr uint64 BcmPacketioManagerTest::kPortId2;
constexpr uint64 BcmPacketioManagerTest::kPortId3;
...
TEST_P(BcmPacketioManagerTest, PushChassisConfigThenVerifySuccessForNode1) {
  ...
  std::map<uint64, std::pair<int, int>> port_id_to_unit_logical_port = {};
  ASSERT_OK(
      PopulateChassisConfigAndPortMaps(&config, &port_id_to_unit_logical_port));
...
TEST_P(BcmPacketioManagerTest, PushChassisConfigFailureForErrorInStartRx) {
  ChassisConfig config;
  ASSERT_OK(PopulateChassisConfigAndPortMaps(&config, nullptr));
...
TEST_P(BcmPacketioManagerTest,
  ...
  mapped_packet_metadata1.set_u32(static_cast<uint32>(kPort1));
  mapped_packet_metadata2.set_type(P4_FIELD_TYPE_EGRESS_PORT);
  mapped_packet_metadata2.set_u32(static_cast<uint32>(kCpuPortId));
...
TEST_P(BcmPacketioManagerTest, TransmitPacketAfterChassisConfigPush) {
  ...
  EXPECT_CALL(*p4_table_mapper_mock_,
      ...
      .WillOnce(DoAll(WithArgs<1>(Invoke([](MappedPacketMetadata* x) {
                      ...
                      Return(::util::OkStatus())));
  EXPECT_CALL(*bcm_chassis_manager_mock_, GetPortState(9999))
  ...
  EXPECT_CALL(*p4_table_mapper_mock_,
      ...
      .WillOnce(DoAll(WithArgs<1>(Invoke([](MappedPacketMetadata* x) {
                        x->set_type(P4_FIELD_TYPE_EGRESS_PORT);
                        x->set_u32(static_cast<uint32>(kPortId1));
                      })),
                      Return(::util::OkStatus())));
  EXPECT_CALL(*bcm_chassis_manager_mock_, GetPortState(kPortId1))
  ...
  // 4- A packet sent to ingress pipeline
