...
::util::Status BfrtCounterManager::WriteIndirectCounterEntry(
  ...
  ASSIGN_OR_RETURN(uint32 table_id, bf_sde_interface_->GetBfRtId(
                                        translated_counter_entry.counter_id()));

  absl::optional<uint64> byte_count;
  absl::optional<uint64> packet_count;
  if (translated_counter_entry.has_data()) {
    byte_count = translated_counter_entry.data().byte_count();
    packet_count = translated_counter_entry.data().packet_count();
  }
  RETURN_IF_ERROR(bf_sde_interface_->WriteIndirectCounter(
      device_, session, table_id, translated_counter_entry.index().index(),
...
::util::Status BfrtCounterManager::ReadIndirectCounterEntry(
  ...
  if (translated_counter_entry.has_index()) {
    optional_counter_index = translated_counter_entry.index().index();
  }
  ...
  for (size_t i = 0; i < counter_indices.size(); ++i) {
    ...
    ::p4::v1::CounterEntry result = translated_counter_entry;
    ...
    ASSIGN_OR_RETURN(*resp.add_entities()->mutable_counter_entry(),
                     bfrt_p4runtime_translator_->TranslateCounterEntry(
                         result, /*to_sdk=*/false));
  }
