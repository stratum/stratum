...
def create_memory_error_alarm_msg(req, *args):
    ...
    req.state_update.memory_error_alarm.severity = common_pb2.Alarm.Severity.Value(
        args[2]
    )
...
def create_flow_programming_exception_alarm_msg(req, *args):
    ...
    req.state_update.flow_programming_exception_alarm.severity = common_pb2.Alarm.Severity.Value(
        args[2]
    )
...
def create_forwarding_viability_msg(req, *args):
    req.state_update.forwarding_viability.state = common_pb2.TrunkMemberBlockState.Value(
        args[0]
    )
    return req
...
def init_args_parser():
    ...
    parser.add_argument(
        "--dry-run", help="Dry run", action="store_const", const=True, default=False
    )
    parser.add_argument(
        "--test-service-url",
        help="Test service address",
        type=str,
        default="localhost:28010",
    )
