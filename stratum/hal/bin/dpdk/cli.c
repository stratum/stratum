...
static int parser_read_uint32(uint32_t* value, const char* p) {
  ...
  if (val > UINT32_MAX) return -ERANGE;
...
static int parser_read_uint16(uint16_t* value, const char* p) {
  ...
  if (val > UINT16_MAX) return -ERANGE;
...
static int parse_tokenize_string(char* string, char* tokens[],
  ...
  if ((string == NULL) || (tokens == NULL) || (*n_tokens < 1)) return -EINVAL;
...
static void print_link_info(struct link* link, char* out, size_t out_size) {
  ...
  snprintf(out, out_size,
           "\n"
           "%s: flags=<%s> mtu %u\n"
           "\tether " RTE_ETHER_ADDR_PRT_FMT
           " rxqueues %u txqueues %u\n"
           "\tport# %u  speed %s\n"
...
static int pipeline_table_entries_add(struct rte_swx_ctl_pipeline* p,
  ...
  for (line_id = 1;; line_id++) {
    ...
    entry = rte_swx_ctl_pipeline_table_entry_read(p, table_name, line,
                                                  &is_blank_or_comment);
    if (!entry) {
      if (is_blank_or_comment) continue;
...
static void cmd_pipeline_table_show(char** tokens, uint32_t n_tokens, char* out,
  ...
  if (status) snprintf(out, out_size, MSG_ARG_INVALID, "table_name");
}
...
static void cmd_pipeline_selector_group_delete(char** tokens, uint32_t n_tokens,
  ...
  status = rte_swx_ctl_pipeline_selector_group_delete(p->ctl, selector_name,
                                                      group_id);
  if (status) snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
}
...
static int pipeline_selector_group_member_read(const char* string,
  ...
  for (s = s0;;) {
    ...
    token = strtok_r(s, " \f\n\r\t\v", &s);
    if (!token || token_is_comment(token)) break;

    if (n_tokens >= GROUP_MEMBER_INFO_TOKENS_MAX) goto error;
   ...
   * Group ID.
   */
  if (parser_read_uint32(&group_id_val, tokens[1]) != 0) goto error;
  *group_id = group_id_val;

  /*
   * Member ID.
   */
  if (parser_read_uint32(&member_id_val, tokens[3]) != 0) goto error;
  *member_id = member_id_val;
  ...
  if (n_tokens && !strcmp(tokens[0], "weight")) {
    if (n_tokens < 2) goto error;

    if (parser_read_uint32(&weight_val, tokens[1]) != 0) goto error;
    *weight = weight_val;
...
error:
  free(s0);
  if (is_blank_or_comment) *is_blank_or_comment = blank_or_comment;
  return -EINVAL;
}
...
static int pipeline_selector_group_members_delete(
    struct rte_swx_ctl_pipeline* p, const char* selector_name, FILE* file,
    uint32_t* file_line_number) {
  char* line = NULL;
  uint32_t line_id = 0;
  int status = 0;
...
static void cmd_pipeline_selector_show(char** tokens, uint32_t n_tokens,
  ...
  selector_name = tokens[3];
  status = rte_swx_ctl_pipeline_selector_fprintf(stdout, p->ctl, selector_name);
  if (status) snprintf(out, out_size, MSG_ARG_INVALID, "selector_name");
}
...
int cli_script_process(const char* file_name, size_t msg_in_len_max,
  ...
  /* Read file */
  for (;;) {
    if (fgets(msg_in, msg_in_len_max + 1, f) == NULL) break;
    ...
    if (strlen(msg_out)) printf("%s", msg_out);
  }
