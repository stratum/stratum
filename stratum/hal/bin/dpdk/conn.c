...
void conn_free(struct conn* conn) {
  ...
  if (conn->fd_client_group) close(conn->fd_client_group);

  if (conn->fd_server) close(conn->fd_server);
...
int conn_poll_for_msg(struct conn* conn) {
  ...
  /* Data available */
  if (event.events & EPOLLIN) status_data = data_event_handle(conn, fd_client);

  /* Control events */
  if (event.events & (EPOLLRDHUP | EPOLLERR | EPOLLHUP))
    status_control = control_event_handle(conn, fd_client);

  if (status_data || status_control) return -1;
