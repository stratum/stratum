...
struct mempool* mempool_find(struct obj* obj, const char* name) {
  ...
  if (strcmp(mempool->name, name) == 0) return mempool;
...
struct link* link_create(struct obj* obj, const char* name,
  ...
  if (rte_eth_dev_info_get(port_id, &port_info) != 0) return NULL;
  ...
  if (rss) {
    ...
    for (i = 0; i < rss->n_queues; i++)
      if (rss->queue_id[i] >= port_info.max_rx_queues) return NULL;
  }
  ...
  if (cpu_id == (uint32_t)SOCKET_ID_ANY) cpu_id = 0;
...
int link_is_up(struct obj* obj, const char* name) {
  ...
  /* Resource */
  if (rte_eth_link_get(link->port_id, &link_params) < 0) return 0;
...
struct link* link_find(struct obj* obj, const char* name) {
  ...
  if (strcmp(link->name, name) == 0) return link;
...
struct ring* ring_create(struct obj* obj, const char* name,
  ...
  /* Check input params */
  if (!name || ring_find(obj, name) || !params || !params->size) return NULL;
...
struct tap* tap_find(struct obj* obj, const char* name) {
  ...
  if (strcmp(tap->name, name) == 0) return tap;
...
struct tap* tap_create(struct obj* obj, const char* name) {
  ...
  /* Check input params */
  if ((name == NULL) || tap_find(obj, name)) return NULL;
...
struct pipeline* pipeline_create(struct obj* obj, const char* name,
  ...
  /* Check input params */
  if ((name == NULL) || pipeline_find(obj, name)) return NULL;
...
struct pipeline* pipeline_find(struct obj* obj, const char* name) {
  ...
  if (strcmp(name, pipeline->name) == 0) return pipeline;
