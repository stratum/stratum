...
static void thread_free(void) {
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    ...
    if (!rte_lcore_is_enabled(i)) continue;

    /* MSGQs */
    if (t->msgq_req) rte_ring_free(t->msgq_req);

    if (t->msgq_rsp) rte_ring_free(t->msgq_rsp);
  }
}
...
int thread_pipeline_enable(uint32_t thread_id, struct obj* obj,
  ...
  /* Check input params */
  if ((thread_id >= RTE_MAX_LCORE) || (p == NULL)) return -1;

  t = &thread[thread_id];
  if (t->enabled == 0) return -1;

  if (!thread_is_running(thread_id)) {
    struct thread_data* td = &thread_data[thread_id];
    struct pipeline_data* tdp = &td->pipeline_data[td->n_pipelines];

    if (td->n_pipelines >= THREAD_PIPELINES_MAX) return -1;
...
int thread_pipeline_disable(uint32_t thread_id, struct obj* obj,
  ...
  if (p->thread_id != thread_id) return -1;
 ...
 */
static inline struct thread_msg_req* thread_msg_recv(
    struct rte_ring* msgq_req) {
  struct thread_msg_req* req;
...
static struct thread_msg_rsp* thread_msg_handle_pipeline_enable(
    struct thread_data* t, struct thread_msg_req* req) {
...
static struct thread_msg_rsp* thread_msg_handle_pipeline_disable(
    struct thread_data* t, struct thread_msg_req* req) {
  struct thread_msg_rsp* rsp = (struct thread_msg_rsp*)req;
...
int thread_main(void* arg __rte_unused) {
  ...
  for (i = 0;; i++) {
    ...
    if ((i & 0xF) == 0) {
      ...
      if (time < t->time_next_min) continue;
      ...
      /* Thread message queues */
      {
        ...
        if (time_next < time_next_min) time_next_min = time_next;
      }
