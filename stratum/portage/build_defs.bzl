...
load("//devtools/build_cleaner/skylark:build_defs.bzl",
...
def sc_platform_filter(value, default, arches):
  return sc_platform_select(host = value if "host" in arches else default,
                            ppc = value if "ppc" in arches else default,
                            x86 = value if "x86" in arches else default)

def sc_platform_alias(name, host=None, ppc=None, x86=None, default=None,
                      visibility=None):
...
def sc_cc_test(name, size = None, srcs = None, deps = None, data = None,
               defines = None, copts = None, linkopts = None, visibility = None):
...
def sc_cc_lib(name, deps = None, srcs = None, hdrs = None, arches = None,
              copts = None, defines = None, includes = None,
              include_prefix = None, strip_include_prefix = None, data = None,
              testonly = None, textual_hdrs = None, visibility = None,
              xdeps = None):
...
def sc_cc_bin(name, deps = None, srcs = None, arches = None, copts = None,
  ...
  native.cc_binary(name = name,
                   ...
                   linkopts = ["-ldl", "-lutil"],
                   testonly = testonly, visibility = visibility)
...
def _gen_proto_lib(name, srcs, hdrs, deps, arch, visibility, testonly,
                   proto_include, grpc_shim_rule):
  ...
  # compile this proto.
  native.filegroup(name = decorate(name_arch, "headers"),
  ...
  proto_rollups = [decorate(decorate(dep, arch), "proto_rollup.flags")
  ...
  for src in srcs + hdrs:
    if src.endswith(".proto"):
      ...
      native.genrule(name = src_arch + ".pb",
                     srcs = protoc_srcs_set,
                     outs = pb_outs, tools = tools,
                     cmd = " && ".join(cmds),
      ...
      if grpc_shim_rule:
        ...
        native.genrule(name = src_arch + ".grpc.pb",
                       srcs = protoc_srcs_set,
                       outs = grpc_pb_outs, tools = grpc_tools,
                       cmd = " && ".join(cmds),
                       heuristic_label_expansion = 0,
                       visibility = visibility)
  ...
  sc_cc_lib(name = decorate(name, arch),
            deps = dep_set,
            srcs = gen_srcs,
            hdrs = hdrs + gen_hdrs,
            arches = [arch],
  ...
  if grpc_shim_rule:
    ...
    sc_cc_lib(name = decorate(grpc_name, arch),
              deps = grpc_dep_set,
              srcs = grpc_gen_srcs,
...
def _gen_proto_shims(name, pb_modifier, srcs, arches, visibility):
  ...
  native.genrule(name = shim_rule, srcs = srcs, outs = outs,
...
def sc_proto_lib(name = None, srcs = [], hdrs = [], deps = [], arches = [],
  ...
  * ${name}_py a py_proto_library version of this library. Only generated
  ...
  deps = depset(deps or [])
  shim_rule = _gen_proto_shims(name = name, pb_modifier = ".pb",
                               srcs = srcs + hdrs, arches = arches,
                               visibility = visibility)
  grpc_shim_rule = None
  if (service_enable["grpc"]):
    grpc_shim_rule = _gen_proto_shims(name = decorate(name[:-6], "grpc_proto"),
                                      pb_modifier = ".grpc.pb",
                                      srcs = srcs + hdrs, arches = arches,
                                      visibility = visibility)
  for arch in arches:
    _gen_proto_lib(name = name, srcs = srcs, hdrs = [shim_rule] + hdrs,
                   deps = deps, arch = arch, visibility = visibility,
                   testonly = testonly, proto_include = proto_include,
                   grpc_shim_rule = grpc_shim_rule)
  if python_support:
    if proto_include:
      fail("Cannot use proto_include on an sc_proto_lib with python support.")
    _gen_py_proto_lib(name = name, srcs = depset(srcs + hdrs), deps = deps,
...
def sc_package(name = None, bins = None, data = None, deps = None,
               arches = None, visibility = None):
  ...
  * ${name}_${arch}_tarball rule builds that .tar.gz package.
  ...
  for extension, inputs in [("bin", ["%s.stripped" % b for b in bins]),
                            ("data", data)]:
    native.Fileset(
