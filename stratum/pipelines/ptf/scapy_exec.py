...
def setup_and_launch_scapy(scapy_dir):
    ...
    EXTERNAL_PATH_SCAPY = (
        ...
        + scapy_dir
        + "/"
        + scapy_dir[6:].replace("_", "-", 1).replace("_", ".")
        + ".data/scripts"
    )
    ...
    scapy_setup_cmd = (
        ...
        + EXEC_PATH_SCAPY
        + " && "
        + "sudo mount -t tmpfs scapy_fs -o size=1M "
        + EXEC_PATH_SCAPY
        + " && "
    ...
    if res == 0:
        scapy_call_cmd = (
            "sudo PYTHONPATH=" + PY_IMPORT_PATH_SCAPY + " " + EXEC_PATH_SCAPY + "/scapy"
        )
        os.system(scapy_call_cmd)

    scapy_umount_cmd = "sudo umount scapy_fs"
