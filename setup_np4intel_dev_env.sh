...
print_help() {
cat << EOF
Builds a docker image using Dockerfile.np4intel.dev and runs a bash session
in it. It is a convenient environment to do Stratum development. The docker
image includes the Bazel build system, git and popular Linux text editors.
This Stratum source directory will be mounted in the docker image. A local
cache directory can be provided to the running docker image so that restarting
the docker does not trigger a complete rebuild of Stratum. The host ssh keys
can also be mounted in the docker to facilitate git usage. The docker image
will take some time to build the first time this script is run.
...
Usage: $0
    ...
    [--bazel-cache <path>]          mount the provided directory into the docker image and use it as the Bazel cache;
                                    default is HOME/.np4intel_cache
...
eval docker build $DOCKER_BUILD_OPTIONS -f $THIS_DIR/Dockerfile.np4intel.dev $THIS_DIR
ERR=$?
if [ $ERR -ne 0 ]; then
